...
windows.inventory =
{
	...
	},
	{
		...
		onRender = function( pos ) if getKeyState( 's' ) then dxDrawRectangle( pos[1], pos[2], pos[3] - pos[1], pos[4] - pos[2], tocolor( unpack( { 72, 0, 191, 63 } ) ) ) end end,
		alignX = "center",
	},
	{
		type = "button",
...
function updateInventory( )
	...
	if t then                                                                                 
		for k, v in ipairs( t ) do
			...
			local TAB = fromJSON(v.value)
			local image = (v.item == 959502 and ":items/images/"..tostring(v.item)..".png") or (v.item == 959501 and ":items/images/"..tostring(v.item)..".png") or (v.item == 5551555 and ':ropas/icons/'..categoria[TAB[3]]..'.png') or (v.item == 5552555 and ':toys/files/'..(TAB[2])..'.png') or exports.items:getImage( v.item, v.value, nItem )
			...
			table.insert( windows.inventory[2].panes,
				{
						...
						dxDrawRectangle( pos[1], pos[2], pos[3] - pos[1], pos[4] - pos[2], tocolor( unpack( getKeyState( 'c' ) and { 255, 0, 0, 63 } or { 72, 0, 191, 63 } ) ) )
						dxDrawRectangle( pos[1] + 64, pos[2] - 52, width2, height, tocolor( 0, 0, 0, 255 ), true )
						dxDrawText( "ID: "..(string.len(tostring(v.value)) < 15 and tostring(v.value) or "Pulsa para más info."), pos[1] + 180, pos[2]+20 , pos[1] + width, pos[2] - height, tocolor( 160, 94, 0, 255 ), 1, "default-bold", "center", "center", false, false, true, postGUI )
						dxDrawText( nItem, pos[1] + 180, pos[2]-25 , pos[1] + width, pos[2] - height, tocolor( 255, 255, 255, 255 ), 1, "default-bold", "center", "center", false, false, true, postGUI )
						end,
			...
			table.insert( windows.inventory_other[2].panes,
				{
					...
					dxDrawRectangle( pos[1], pos[2], pos[3] - pos[1], pos[4] - pos[2], tocolor( unpack( getKeyState( 'c' ) and { 255, 0, 0, 63 } or { 72, 0, 191, 63 } ) ) )
					dxDrawRectangle( pos[1] + 64, pos[2] - 52, width2, height, tocolor( 0, 0, 0, 255 ), true )
					dxDrawText( "ID: "..tostring(v.value), pos[1] + 180, pos[2]+20 , pos[1] + width, pos[2] - height, tocolor( 160, 94, 0, 255 ), 1, "default-bold", "center", "center", false, false, true, postGUI )
					dxDrawText( nItem, pos[1] + 180, pos[2]-25 , pos[1] + width, pos[2] - height, tocolor( 255, 255, 255, 255 ), 1, "default-bold", "center", "center", false, false, true, postGUI )
					end,
					image = image or ":players/images/skins/-1.png",
					onClick = function( key )
