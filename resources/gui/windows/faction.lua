...
local rightNames = { [0] = "-", [1] = "Lider", [2] = "Dueño" }
local rankNames = { }
...
function updateFaction( fnum, members, presupuesto, name, ranks )
	...
	windows.faction[4].text = "Presupuesto: $ "..tostring( formatNumber( presupuesto, "," ) )..""
	local grid = { }
	local ownRights = 0
	...
	for key, value in ipairs( members ) do
		...
		if ownRights == 2 or duty == true then
			...
			local sueldo = tonumber(guiGetText( destroy[ "g:faction_sueldo:cantidad" ] ))
				...
				if fnum == 1 or fnum == 51 then 

					if sueldo and math.floor(sueldo) >= 10000 and math.floor(sueldo) <= 650000 then
						hide( )
						...
						outputChatBox ( "[ La Capital Roleplay - Facciones ]: El sueldo debe estar entre $10,000 y $650,000, ¡modifícalo!", 255, 0, 0, true )
	
					end

				elseif fnum == 5 or fnum == 10 or fnum == 11 or fnum == 12 or fnum == 14 or fnum == 15 or fnum == 16 then 

					if sueldo and math.floor(sueldo) >= 1500000 and math.floor(sueldo) <= 6000000 then
						hide( )
						triggerServerEvent( "faction:cambiarsueldo", localPlayer, fnum, getElementData(localPlayer, "nsueldo"), math.floor(sueldo) )
						triggerServerEvent( "faction:show", localPlayer, -fnum )
					else 
						
						outputChatBox ( "[ La Capital Roleplay - Facciones ]: El sueldo debe estar entre $1,500,000 y $6,000,000, ¡modifícalo!", 255, 0, 0, true )
	
					end

				else 
					
					if sueldo and math.floor(sueldo) >= 5000 and math.floor(sueldo) <= 100000 then
						hide( )
						triggerServerEvent( "faction:cambiarsueldo", localPlayer, fnum, getElementData(localPlayer, "nsueldo"), math.floor(sueldo) )
						triggerServerEvent( "faction:show", localPlayer, -fnum )

					else
						outputChatBox ( "[ La Capital Roleplay - Facciones ]: El sueldo debe estar entre $5,000 y $100,000, ¡modifícalo!", 255, 0, 0, true )
					end
			...
			local text = "Mínimo $5,000 y máximo $75,000"

			if fnum == 1 or fnum == 109 then
				text = "Mínimo $10,000 y Máximo $650,000"
			elseif fnum == 5 or fnum == 10 or fnum == 11 or fnum == 12 or fnum == 14 or fnum == 15 or fnum == 16 then
				text = "Mínimo $1,500,000 y Máximo $6,000,000"
			end
			...
			windows.faction_sueldo =
		{
			...
			table.insert( t, tostring("$"..formatNumber( sueldo, "," )) )
		end
	...
	for k, v in ipairs( buttons ) do
		...
		a.onHover = function( ) a.color = { 203, 50, 52 } end
