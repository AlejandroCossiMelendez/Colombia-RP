...
function updateShopContent( content, name )
	...
	for k, value in ipairs( content ) do
		...
		if value.itemID and tonumber(value.itemID) == 29 then
			table.insert( windows.shop[2].panes,
				{
					...
					onHover = function( cursor, pos )
							dxDrawRectangle( pos[1], pos[2], pos[3] - pos[1], pos[4] - pos[2], tocolor( unpack( { 0, 255, 255, 31 } ) ) )
						end,
					onClick = function( key )
							if key == 1 then
			...
			table.insert( windows.shop[2].panes,
				{
					image = (value.itemID == 959502 and ":items/images/"..tostring(value.itemID)..".png") or (value.itemID == 959501 and ":items/images/"..tostring(value.itemID)..".png") or exports.items:getImage( value.itemID, value.itemValue, value.name ) or ":players/images/skins/-1.png",
