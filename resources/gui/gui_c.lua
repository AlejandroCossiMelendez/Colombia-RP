...
local basic_font = dxCreateFont( "dxGUI/fonts/basic_bold.ttf", 11 )
local basic_font1 = dxCreateFont( "dxGUI/fonts/basic.ttf", 10 )
...
local function draw( window, y )
	if window.type == "grid" then
		...
		for k, v in ipairs( window.columns ) do
			...
			dxDrawText( tostring( v.name ), v.pos._start, y, v.pos._end, y + line_height, v.color and tocolor( unpack( v.color ) ) or tocolor( 255, 255, 255, 255 ), 1, basic_font, v.alignX or "left", v.alignY or "center", true, false, true, true)
		end
		...
		for key, value in ipairs( window.cachedcontent ) do
			...
			for k, text in ipairs( value ) do
				...
				-- draw it
				dxDrawText( tostring( text ), v.pos._start, y, v.pos._end, y + line_height, color and tocolor( unpack( color ) ) or v.color and tocolor( unpack( v.color ) ) or tocolor( 255, 255, 255, 255 ), 1, v.font or "default-bold", v.alignX or "left", v.alignY or "center", true, false, true, true )
			end
	...
	elseif window.type == "label" or window.type == "edit" then
		...
		dxDrawText( tostring( window.cachedtext ), x, y, x + width / ( window.type == "edit" and 2.7 or 1 ), y + line_height, window.color and tocolor( unpack( window.color ) ) or tocolor( 255, 255, 255, 255 ), window.scale or 1, window.font or "default", window.alignX or window.type == "edit" and "right" or "left", window.alignY or "center", true, false, true, true )
	...
	elseif window.type == "pane" then
		for key, value in ipairs( window.cachedpanes ) do
			...
			dxDrawText( value.title, x + 65, y, x + width, y + ( value.text and 18 or 64 ), tocolor( 255, 255, 255, 255 ), 1, basic_font, "left", value.text and "top" or "center", true, false, true, true )
			if value.text ~= nil then
				dxDrawText( tostring( type( value.text ) == "function" and value.text( ) or value.text ), x + 70, y + 18, x + width, y + 64, tocolor( 255, 255, 255, 255 ), 1, basic_font1, "left", "top", true, value.wordBreak or false, true, postGUI )
			end
			y = y + 65
		end
	elseif window.type == "vpane" then
...
addEventHandler( "onClientRender", root,
	function( )
		if window then
			...
			DxDrawBorderedRectangle( x - 5, window.snapTop and 0 or ( y - 5 ), width + 10, window.snapTop and window.snapBottom and screenY or not window.snapTop and window.snapBottom and ( screenY - x + 5 ) or ( height + 10 ), tocolor( 17, 17, 25 ), tocolor( 32, 34, 46 ), 2, false )
			...
			if not forcedWindow then
				...
				if key then
					dxDrawRectangle( x - 35, screenY / 2 - 15, 30, 30, tocolor( 0, 0, 0, 127 ), false )
					dxDrawText( "<", x - 35, screenY / 2 - 15, x - 5, screenY / 2 + 15, tocolor( 255, 255, 255, 255 ), 1, "bankgothic", "center", "center", true, false, true, postGUI )
					...
					dxDrawRectangle( x + width + 5, screenY / 2 - 15, 30, 30, tocolor( 0, 0, 0, 127 ), false )
					dxDrawText( ">", x + width + 5, screenY / 2 - 15, x + width + 35, screenY / 2 + 15, tocolor( 255, 255, 255, 255 ), 1, "bankgothic", "center", "center", true, false, true, true )
...
function show( name, forced, dontEnableInput, mouse, ignored )
	...
	-- No mostrar characters si estamos en proceso de spawn directo
	if name == "characters" and getElementData(getLocalPlayer(), "direct_spawn_process") then
		outputDebugString("[GUI] Bloqueando interfaz characters - spawn directo activo")
		return
	end
