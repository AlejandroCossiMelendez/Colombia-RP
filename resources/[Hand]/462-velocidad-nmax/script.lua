...
if getElementModel(v) == 00 then --ID do Veiculo que deseja setar a hand
	setVehicleHandling(v, "mass", 4000) --Massa do veículo no caso 1600
	...
	setVehicleHandling(v, "maxVelocity", 225) --Velocidade Máxima
	setVehicleHandling(v, "engineAcceleration", 47) -- Aceleração do Veículo
	setVehicleHandling(v, "engineInertia", 10)
	setVehicleHandling(v, "driveType", "awd")
	setVehicleHandling(v, "engineType", "petrol")
	setVehicleHandling(v, "brakeDeceleration", 200) --Força do freio
	...
	--==MOTO==-- 	
if getElementModel(v) == 462 then --ID do Veiculo que deseja setar a hand
