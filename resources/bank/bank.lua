...
addEventHandler( "onResourceStart", resourceRoot,
	function( )
		...
		-- Inicializar ElementData para todos los jugadores conectados
		for _, player in ipairs(getElementsByType("player")) do
			if exports.players:isLoggedIn(player) then
				updateBankElementData(player)
			end
		end
	end
)

-- Función para actualizar ElementData del banco
function updateBankElementData(player)
    if exports.players:isLoggedIn(player) then
        ...
        if sql and sql.banco then
            setElementData(player, "Banco", tonumber(sql.banco))
            return true
        else
            setElementData(player, "Banco", 0)
            return true
        end
    end
    return false
...
-- Inicializar ElementData al login
function initPlayerBankData(player)
    updateBankElementData(player)
end
addEvent("onPlayerLogin", true)
addEventHandler("onPlayerLogin", getRootElement(), initPlayerBankData)
...
function getDinero(player)
	if exports.players:isLoggedIn(player) then
		local sql = exports.sql:query_assoc_single("SELECT banco FROM characters WHERE characterID = " .. exports.players:getCharacterID(player))
		if sql and sql.banco then
			local balance = tonumber(sql.banco)
			setElementData(player, "Banco", balance) -- Establecer ElementData aquí
			return balance
		else
			setElementData(player, "Banco", 0) -- Establecer a 0 si no se encuentra saldo
			return 0
		end
	else 
		return false 
...
function ingresarDinero(player, cmd, cantidad)
	if exports.players:isLoggedIn(player) then
		...
		if exports.players:takeMoney(player, cantidad) then
			local nuevoSaldo = getDinero(player) + cantidad
			if exports.sql:query_free("UPDATE characters SET banco = " .. nuevoSaldo .. " WHERE characterID = " .. exports.players:getCharacterID(player)) then
				setElementData(player, "Banco", nuevoSaldo) -- Actualizar ElementData
				...
				outputChatBox("Balance actual: $" .. tostring(nuevoSaldo) .. ".", player, 0, 255, 0)
				...
				if nivel == 2 and nuevoSaldo >= 10000 and not exports.objetivos:isObjetivoCompletado(17, exports.players:getCharacterID(player)) then
					exports.objetivos:addObjetivo(17, exports.players:getCharacterID(player), player)
				end
				if nivel == 3 and nuevoSaldo >= 50000 and not exports.objetivos:isObjetivoCompletado(22, exports.players:getCharacterID(player)) then
					exports.objetivos:addObjetivo(22, exports.players:getCharacterID(player), player)
				end
				if nivel == 3 and nuevoSaldo >= 100000 and not exports.objetivos:isObjetivoCompletado(31, exports.players:getCharacterID(player)) then
					exports.objetivos:addObjetivo(31, exports.players:getCharacterID(player), player)
				end
				if nivel == 4 and nuevoSaldo >= 200000 and not exports.objetivos:isObjetivoCompletado(34, exports.players:getCharacterID(player)) then
					exports.objetivos:addObjetivo(34, exports.players:getCharacterID(player), player)
				end
				if nivel == 4 and nuevoSaldo >= 300000 and not exports.objetivos:isObjetivoCompletado(35, exports.players:getCharacterID(player)) then
...
function consultarSaldo(player)
	if not bancoCercano(player) then outputChatBox("No estás cerca de un cajero o de un dependiente del banco.", player, 255, 0, 0) return end
	if not getDinero(player) then outputChatBox("Se ha producido un error, inténtalo de nuevo más tarde.", player, 255, 0, 0) return end
	local saldo = getDinero(player)
	outputChatBox("El balance de tu cuenta es de $" .. tostring(saldo) .. ".", player, 0, 255, 0)
end
	...
	if (source == seguridadte) then
		if getElementType(player) == "player" and not isPedInVehicle(player) then
			...
			if exports.factions:isPlayerInFaction(player, 1) then 
				outputChatBox("ATENCIÓN: Te has identificado como agente del sheriff.", player, 0, 255, 0) 
				return 
			end
