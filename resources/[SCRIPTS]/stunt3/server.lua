...
addCommandHandler("stunt3",
	function(player)
		if (not animEnable[player]) then
			animEnable[player] = true
			triggerClientEvent(syncPlayers, "empe", player, true)
			outputChatBox("", player, 0, 255, 0, true)
		else
			animEnable[player] = false
			triggerClientEvent(syncPlayers, "empe", player, false)
...
addEventHandler("onClientSync", resourceRoot,
    function()
		...
		for player, enable in ipairs(animEnable) do
			if (enable) then
				triggerClientEvent(client, "empe", player, true)
			end
		end
    end 
)
