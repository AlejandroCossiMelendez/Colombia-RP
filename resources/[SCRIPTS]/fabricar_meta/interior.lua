...
local marker = createMarker(1941.791015625, 156.541015625, 37.083686828613 -1, "cylinder", 1.20, 30, 145, 255, 100)
local int = 0
local dim = 0
...
local drog2, slot2, v = exports.items:has(playerSource, 37)
	...
	local x2, y2, z2 = getElementPosition(marker)
		if getDistanceBetweenPoints3D(x, y, z, x2, y2, z2) <= 3 then
			...
			setTimer(function()
				...
				exports.items:give(playerSource, 21, 1)
				exports.items:give(playerSource, 21, 1)
				exports.infobox:addNotification(playerSource, "Has fabricado metanfetamina", "success")
				...
				for _,v in ipairs(getElementsByType("player")) do 
					if exports.factions:isPlayerInFaction(v, 1) or exports.factions:isPlayerInFaction(v, 1) then
						outputChatBox("#FFFFFF[#42B007Entorno#FFFFFF] (#42B007"..name.."#FFFFFF): Se sentiria un fuerte olor salir de la comuna de cali.", v, 255, 255, 255, true) --efeito no chat
					end
				end
...
exports.infobox:addNotification(playerSource, "Para fabricar metanfetamina deben haber 2 policias en servicio", "error")
end
end
