...
function recibirTablasNecesarias (tabla_sms, tabla_contactos, tabla_vehiculos, tabla_vehsPedir, saldo_cuenta, texto_agenda)
	tablaSMS = tabla_sms
	tablaContactos = tabla_contactos
	tablaVehiculos = tabla_vehiculos
	tablaVehsPedir = tabla_vehsPedir
...
pages['home'].base_pedirveh = guiCreateStaticImage( 5+margin_x*0, 5+margin_y*3, appSize, appSize, "images/app_vehicle.png", false, background)
pages['home'].base_off = guiCreateStaticImage( 5+margin_x*1, 5+margin_y*3, appSize, appSize, "images/app_off.png", false, background)
...
pages['home'].base_nequi = guiCreateStaticImage ( 5+margin_x*2, 5+margin_y*2, appSize, appSize, "images/app_nequi.png", false, background )
...
setElementData ( pages['home'].base_contacts, "tooltip-text", "Mandar Plata" )
...
setElementData ( pages['home'].base_nequi, "tooltip-text", "Nequi" )
...
setElementData ( pages['home'].base_pedirveh, "tooltip-text", "Pedir mi Vehiculo" )
...
setElementData ( pages['home'].base_nequi, "tooltip-displaytext", false )
...
setElementData ( pages['home'].base_pedirveh, "tooltip-displaytext", false)
...
addEventHandler( "onClientGUIClick", getRootElement(),
	function()
		if guiGetVisible( source ) then
			if source == pages['calling_r'].colgar or source == pages['calling'].colgar then
...
-- Nequi x PandFort Developer ez Sable kkkkkkkkk

pages['nequi'] = { }
pages['nequi'].font = guiCreateFont("nequi.ttf", 10)

pages['nequi'].bk = guiCreateStaticImage( 43, 92, 262, 422, "images/backgrounds/nequi_background.png", false, base )
pages['nequi'].pName = guiCreateLabel(47, 10, 114, 32, getPlayerName( localPlayer ):gsub( "_", " " ), false, pages['nequi'].bk)
guiSetFont(pages['nequi'].pName, pages['nequi'].font)
guiLabelSetVerticalAlign(pages['nequi'].pName, "center")

pages['nequi'].dinero = guiCreateLabel(83, 57, 114, 32, "999,999,999 $", false, pages['nequi'].bk)
guiSetFont( pages['nequi'].dinero, pages['nequi'].font)
guiLabelSetHorizontalAlign(pages['nequi'].dinero, "center", false)
guiLabelSetVerticalAlign( pages['nequi'].dinero, "center")
        pages['nequi'].textBoxID = guiCreateLabel(93, 190, 120, 25, "4", false, pages['nequi'].bk)
        guiLabelSetColor(pages['nequi'].textBoxID, 0, 0, 0)
        guiLabelSetHorizontalAlign(pages['nequi'].textBoxID, "left", false)
        guiLabelSetVerticalAlign(pages['nequi'].textBoxID, "center")
        pages['nequi'].textCNT = guiCreateLabel(93, 236, 120, 25, "4", false, pages['nequi'].bk)
        guiLabelSetColor(pages['nequi'].textCNT, 1, 0, 0)
        guiLabelSetHorizontalAlign(pages['nequi'].textCNT, "left", false)
        guiLabelSetVerticalAlign(pages['nequi'].textCNT, "center")
        pages['nequi'].Button = guiCreateButton(5, 364, 248, 33, "", false, pages['nequi'].bk)
        guiSetAlpha(pages['nequi'].Button, 0.00)
        pages['nequi'].box1 = guiCreateEdit(94, 189, 119, 26, "", false, pages['nequi'].bk)
        guiSetAlpha(pages['nequi'].box1, 0.00)
        pages['nequi'].box2 = guiCreateEdit(94, 235, 119, 26, "", false, pages['nequi'].bk)
        guiSetAlpha(pages['nequi'].box2, 0.00)  
...
function( )
	cargado = loadBrowserURL( pages['youtube'].theBrowser, "https://m.youtube.com/?persist_app=1&app=m" )
	setTimer(function ()
		if not cargado then 
			cargado = loadBrowserURL( pages['youtube'].theBrowser, "https://m.youtube.com/?persist_app=1&app=m")
		end
...
addCommandHandler("youtube",function ()
	reloadBrowserPage(pages['youtube'].theBrowser)
end)

local screenWidth, screenHeight = guiGetScreenSize()
local browserVolume = 1.0

function barravolumen()
	local panelWindow = guiCreateWindow(0.56, 0.68, 0.19, 0.12, "Control de Volumen", true)
	guiWindowSetSizable(panelWindow, false)
	--local vayaVol = guiGetVisible(panelWindow)
	--setElementData(localPlayer, "activoVol", vayaVol)

local cerrar = guiCreateButton(173, 25, 59, 15, "Cerrar", false, panelWindow)
local volumeLabel = guiCreateLabel(10, 30, 160, 20, "Volumen: 1.0", false, panelWindow)

local volumeSlider = guiCreateScrollBar(10, 46, 222, 13, true, false, panelWindow)
guiScrollBarSetScrollPosition(volumeSlider, browserVolume * 100)

addEventHandler("onClientGUIClick", cerrar, 
function()
  guiSetVisible(panelWindow, false)
end)

function llamarVolumen()
	if guiGetVisible(panelWindow) == false then
		guiSetVisible(panelWindow, true)
 end
end

function cerrarVolumen()
    guiSetVisible(panelWindow, false)
end

local function updateBrowserVolume()
    local newVolume = guiScrollBarGetScrollPosition(volumeSlider) / 100
    browserVolume = newVolume
    guiSetText(volumeLabel, "Volumen: " .. string.format("%.1f", newVolume))
    setBrowserVolume(pages['youtube'].theBrowser, newVolume)
end
addEventHandler("onClientGUIScroll", volumeSlider, updateBrowserVolume, false)
local function syncVolumeSlider()
    guiScrollBarSetScrollPosition(volumeSlider, browserVolume * 100)
 end
  syncVolumeSlider()
  end
...
-- Vehicles
pages['pedir_vehicles'] = { }
pages['pedir_vehicles'].grid = guiCreateGridList ( 4, 11, 257, 330, false, background )
pages['pedir_vehicles'].locate = guiCreateButton ( 4, 383, 250, 25, "Pedir Veh", false, background )

guiGridListAddColumn ( pages['pedir_vehicles'].grid, "ID", 0.3 )
guiGridListAddColumn ( pages['pedir_vehicles'].grid, "Modelo", 0.6 )
...
function onPhoneRender ( )
	...
	phone_x = x
	phone_y = y
	dxDrawFixedText ( "Tigo",  x, y+78, 350, 20, tocolor ( 255, 255, 255, 255 ), 1, "default-bold", "center", "top", false, false, true )
	Draw2048Images ( )
	if ( LoadedPage == "nequi" ) then
		guiSetText( pages['nequi'].textBoxID, guiGetText( pages['nequi'].box1 ) )
		guiSetText( pages['nequi'].textCNT, guiGetText( pages['nequi'].box2 ) )
		guiSetText( pages['nequi'].dinero, getPlayerMoney( localplayer ) .. " $" )
	end
...
function clickingHandler ( s, b, x, y )
	if ( s == "left" and b == "up" ) then
		-- home page
		if isMouseInPosition ( 1732, 998, 61, 58 ) or ( x >= phone_x + 160 and x <= phone_x + 190 and y >= phone_y + 522 and y <= phone_y + 545 ) then
			if ( LoadedPage ~= "home" ) then
...
function clickingHandler2 ( b, s )
	...
	if ( LoadedPage == "home" ) then
		...
		elseif ( source == pages['home'].base_nequi ) then
			

			guiSetVisible( background, false )
			setPhonePageOpen ( "nequi" )
			guiMemoSetReadOnly ( pages['nequi'].texto, true )
			guiSetText(pages['nequi'].texto, "¡Bienvenido a Nequi!\nPodrás realizar envíos de dinero sin comisión.\nEl balance de tu cuenta es $"..formatNumber ( getPlayerMoney ( localPlayer ), "," ) )
		...
		elseif ( source == pages['home'].base_notes ) then
			...
			--if getElementData(localPlayer, "activoVol") == false then
			barravolumen()
		   -- end
		...
		elseif ( source == pages['home'].base_pedirveh ) then
			setPhonePageOpen ( "pedir_vehicles" )
			guiGridListClear(pages['pedir_vehicles'].grid)
			for i, v in ipairs ( tablaVehsPedir ) do                       
				local r = guiGridListAddRow ( pages['pedir_vehicles'].grid )
				guiGridListSetItemText ( pages['pedir_vehicles'].grid, r, 1, tostring ( v.vehicleID ), false, false )
				guiGridListSetItemText ( pages['pedir_vehicles'].grid, r, 2, tostring ( getVehicleNameFromModel(v.model) ), false, false )
				guiGridListSetItemData ( pages['pedir_vehicles'].grid, r, 1, tonumber(v.vehicleID) )
			end
	...
	elseif ( LoadedPage == "sms" ) then
		...
		-- Nequi función | Sable Cortado
	elseif ( ( LoadedPage == "nequi" ) ) then 
		
		-- guiSetVisible( background, false )

		if ( source == pages['nequi'].Button ) then

			local destino = guiGetText ( pages['nequi'].textBoxID )
			local dinero = guiGetText ( pages['nequi'].textCNT )
			...
			if dinero ~= "" and destino ~= "" then 
				...
				if dinero then
					...
					if tonumber (dinero) >= 1 and tonumber (dinero) <= 5000000 then 
						...
						triggerServerEvent ( "accionNequi", localPlayer, localPlayer, destino, dinero )

					else 

						outputChatBox ( "#7000FF[Nequi]: #FF0000¡Introduce una cantidad mayor que $1 y menor que $5,000,000!", 255, 255, 255, true )
						playSound ( "https://cdn.discordapp.com/attachments/919681842041458742/957539760195395594/error_tonal3_1.mp3" )

					end 
				
				else 
					outputChatBox ( "#7000FF[Nequi]: #FF0000¡El monto debe ser un número, comprúebalo!", 255, 255, 255, true )
					playSound ( "https://cdn.discordapp.com/attachments/919681842041458742/957539760195395594/error_tonal3_1.mp3" )
				...
				outputChatBox ( "#7000FF[Nequi]: #FF0000¡No puedes dejar campos vacíos, verifícalo!", 255, 255, 255, true )
				playSound ( "https://cdn.discordapp.com/attachments/919681842041458742/957539760195395594/error_tonal3_1.mp3" )
	...
	elseif ( LoadedPage == "vehicles" ) then
	 ...
	 elseif ( LoadedPage == "pedir_vehicles" ) then
		if ( source == pages['pedir_vehicles'].locate ) then
			local row, col = guiGridListGetSelectedItem ( pages['pedir_vehicles'].grid )
			if ( row ~= -1 ) then
				local vehicleID = guiGridListGetItemData ( pages['pedir_vehicles'].grid, row, 1 )
				triggerServerEvent ( "onPedirVehiculo", localPlayer, localplayer, "lveh", tonumber(vehicleID))
			end
		end
...
addEvent ( "sonidosT", true )
addEventHandler ( "sonidosT", localPlayer, 
	function ( tipo )
		if tipo == 1 then 
			playSound ( "https://cdn.discordapp.com/attachments/919681842041458742/957539852042240030/send_tonal1.mp3" )
		elseif tipo == 2 then 
			playSound ( "https://cdn.discordapp.com/attachments/952418636432293958/957533050512748574/send_tonal2.mp3" )
		elseif tipo == 3 then 
			playSound ( "https://cdn.discordapp.com/attachments/952418636432293958/957529826225455144/error_tonal3_1.mp3" )
		end
	end 
)
