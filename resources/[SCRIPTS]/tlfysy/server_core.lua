...
local services =
{
	[112] =
	{
		function( player, phoneNumber, input )
			if player then
				...
				return true, "Emergencias: Si necesita policias ((/p 1)), si necesita médicos ((/p 2)) , si necesita seguridad privada ((/p 3)) o si necesita un militar ((/p 4))"
			end
		end,
		true,
		function( player, phoneNumber, input )
			if input[1] == "1" or string.lower(input[1]) == "uno" then
				...
				if #policias > 0 then
					sElegido[player] = 1
					return true, "Emergencias: Departamento de Policia, indique el motivo de su emergencia"
				else
			...
			elseif input[1] == "2" or string.lower(input[1]) == "dos" then
				...
				if #medicos > 0 then
					sElegido[player] = 2
					return true, "Emergencias: Departemento medico de ASSE, indique el motivo de su emergencia"
				else
			...
			elseif input[1] == "3" or string.lower(input[1]) == "tres" then
				local bomberos = getJugadoresFaccion ( 6 )
				if #bomberos > 0 then
					sElegido[player] = 3
					return true, "Emergencias: Bomberos Uruguay, indique el motivo de su emergencia"
				else
		...
		function( player, phoneNumber, input )
			...
			elseif sElegido[player] == 3 then
				...
				for key, value in ipairs( bomberos ) do
					outputChatBox( "Emergencia - Telefono " .. phoneNumber .. " - Situacion: "..tostring(input[2])..".", value, 130, 255, 130 )
					triggerClientEvent( value, "gui:hint", value, "Emergencia", "\nTelefono: " .. phoneNumber .. " \nSituacion: "..tostring(input[2]).."." )
					exports.factions:createFactionBlip2(x, y, z, 6)
				end
				colgarLlamada(player)
				return false, "Emergencias: Un equipo de bomberos se dirige a su posicion, mantenga la calma."
					elseif input[1] == "4" or string.lower(input[1]) == "cuatro" then
				local militar = getJugadoresFaccion ( 16 )
				if #militar > 0 then
					sElegido[player] = 4
					return true, "Emergencias: Ejercito nacional, indique el motivo de su emergencia"
				else
					return false, "Emergencias: En este momento no hay ningun militar disponible, ¡llame mas tarde!"
					end								
			end
		end
	},
	[911] =
	{
		function( player, phoneNumber, input )
			if player then
				...
				return true, "Emergencias: Si necesita policias ((/p 1)), si necesita médicos ((/p 2)) , si necesita bomberos ((/p 3)) o si necesita un militar ((/p 4))"
			end
		end,
		true,
		function( player, phoneNumber, input )
		...
		function( player, phoneNumber, input )
			...
			elseif sElegido[player] == 3 then
				...
				colgarLlamada(player)
				return false, "Emergencias: Un equipo de bomberos se dirige a su posicion, mantenga la calma."
			elseif sElegido[player] == 4 then
				local policias = getJugadoresFaccion ( 16 )
				for key, value in ipairs( policias ) do
					outputChatBox( "Emergencia - Telefono " .. phoneNumber .. " - Situacion: "..tostring(input[2])..".", value, 130, 255, 130 )
					triggerClientEvent( value, "gui:hint", value, "Emergencia", "\nTelefono: " .. phoneNumber .. " \nSituacion: "..tostring(input[2]).."." )
					exports.factions:createFactionBlip2(x, y, z, 16)
				end
				return false, "Emergencias: Recibido, enseguida mandamos un militar a su posición. Mantenga la calma."
			end
		end
	},
	[555] =
	{
		function( player, phoneNumber, input )
			...
			if #mecanicos > 0 then
				return true, "ACU operador dice: Ha llamado usted al servicio de talleres, explique brevemente el motivo de su llamada."
			else
				colgarLlamada(player)
				return false, "ACU operador dice: En este momento no hay ningun mecanico disponible, ¡llame mas tarde!"
			end	
		end,
		true,
		function( player, phoneNumber, input )
			...
			if #mecanicos > 0 then
				...
				colgarLlamada(player)
				return false, "ACU Operador dice: Un asistente de grua se dirige a su posicion, permanezca alli."
			else
				colgarLlamada(player)
				return false, "ACU Operador dice: En estos momentos no hay ningun mecanico disponible, ¡llame mas tarde!"
			end
		end
	},
	[666] =
	{
		function( player, phoneNumber, input )
			local juzgados = getJugadoresFaccion ( 15 )
			if #juzgados > 0 then
				return true, "Cymaco operador dice: Ha llamado usted al servicio de talleres, explique brevemente el motivo de su llamada."
			else
				colgarLlamada(player)
				return false, "Cymaco operador dice: En este momento no hay ningun mecanico disponible, ¡llame mas tarde!"
			end	
		end,
		true,
		function( player, phoneNumber, input )
			...
			if #juzgados > 0 then
				for key, value in ipairs( juzgados ) do
					outputChatBox( "Aviso de Taller: Telefono " .. phoneNumber .. " - Situacion: " .. input[1], value, 130, 255, 130 )
					triggerClientEvent( value, "gui:hint", value, "Aviso de Cymaco", "\nTelefono: " .. phoneNumber .. " \nSituacion: " .. input[1] )
					x, y, z = getElementPosition(player)
					exports.factions:createFactionBlip2(x, y, z, 15)
				end
				colgarLlamada(player)
				return false, "Cymaco Operador dice: Un asistente de grua se dirige a su posicion, permanezca alli"
			else
				colgarLlamada(player)
				return false, "Cymaco Operador dice: En estos momentos no hay ningun mecanico disponible, ¡llame mas tarde!"
			end
		end
	},
	[222] =
	{
		function( player, phoneNumber, input )
			...
			if #juzgados > 0 then
				return true, "Operador dice: Ha llamado usted a la central de Taxis, indique el motivo de su llamada."
			else
				colgarLlamada(player)
				return false, "Operador dice: En este momento no hay taxistas disponibles, ¡llame mas tarde!"
			end	
		end,
		true,
		function( player, phoneNumber, input )
			...
			if #juzgados > 0 then
				for key, value in ipairs( juzgados ) do
					outputChatBox( "Aviso de Taxis: Telefono " .. phoneNumber .. " - Situacion: " .. input[1], value, 130, 255, 130 )
					triggerClientEvent( value, "gui:hint", value, "Aviso de Taxista", "\nTelefono: " .. phoneNumber .. " \nSituacion: " .. input[1] )
					x, y, z = getElementPosition(player)
					exports.factions:createFactionBlip2(x, y, z, 8)
				end
				colgarLlamada(player)
				return false, "Operador dice: De acuerdo, enseguida enviamos un taxista a su posicion,¡ESPER EN ESE LUGAR!"
			else
				colgarLlamada(player)
				return false, "Operador dice: En este momento no hay taxistas disponibles, ¡llame mas tarde!"
			end
	...
	{
		function( player, phoneNumber, input )
			local pd = getJugadoresFaccion ( 1 )
			if #pd > 0 then
				return true, "PD operador dice: Ha llamado usted al departamento de policia, explique brevemente el motivo de su llamada."
			else
				colgarLlamada(player)
				return false, "PD operador dice: En este momento no hay ningun policia disponible, ¡llame mas tarde!"
			end	
		end,
		true,
		function( player, phoneNumber, input )
			local pd = getJugadoresFaccion ( 1 )
			if #pd > 0 then
				for key, value in ipairs( pd ) do
					outputChatBox( "Aviso de Policia - Telefono " .. phoneNumber .. " - Situacion: " .. input[1], value, 130, 255, 130 )
					triggerClientEvent( value, "gui:hint", value, "Aviso del Policia", "\nTelefono: " .. phoneNumber .. " \nSituacion: " .. input[1] )
					x, y, z = getElementPosition(player)
					exports.factions:createFactionBlip2(x, y, z, 1)
				end
				colgarLlamada(player)
				return false, "PD Operador dice: Un asistente de policia se dirige a su posicion, permanezca alli."
			else
				colgarLlamada(player)
				return false, "PD Operador dice: En estos momentos no hay ningun policia disponible, ¡llame mas tarde!"
			end
		end
	},
	...
	{
		function( player, phoneNumber, input )
			local ej = getJugadoresFaccion ( 16 )
			if #ej > 0 then
				return true, "Ejercito operador dice: Ha llamado usted al Ejercito nacional, explique brevemente el motivo de su llamada."
			else
				colgarLlamada(player)
				return false, "Ejercito operador dice: En este momento no hay ningun militar disponible, ¡llame mas tarde!"
			end	
		end,
		true,
		function( player, phoneNumber, input )
			local ej = getJugadoresFaccion ( 16 )
			if #ej > 0 then
				for key, value in ipairs( ej ) do
					outputChatBox( "Aviso de Ejercito - Telefono " .. phoneNumber .. " - Situacion: " .. input[1], value, 130, 255, 130 )
					triggerClientEvent( value, "gui:hint", value, "Aviso del Ejercito", "\nTelefono: " .. phoneNumber .. " \nSituacion: " .. input[1] )
					x, y, z = getElementPosition(player)
					exports.factions:createFactionBlip2(x, y, z, 16)
				end
				colgarLlamada(player)
				return false, "Ejercito Operador dice: Un asistente de milicia se dirige a su posicion, permanezca alli."
			else
				colgarLlamada(player)
				return false, "Ejercito Operador dice: En estos momentos no hay ningun militar disponible, ¡llame mas tarde!"
			end
		end
	},
	...
	{
		function( player, phoneNumber, input )
			local gm = getJugadoresFaccion ( 9 )
			if #gm > 0 then
				return true, "Gamma operador dice: Ha llamado usted a Seguridad Privada, explique brevemente el motivo de su llamada."
			else
				colgarLlamada(player)
				return false, "Gamma operador dice: En este momento no hay ningun guardia disponible, ¡llame mas tarde!"
			end	
		end,
		true,
		function( player, phoneNumber, input )
			local gm = getJugadoresFaccion ( 9 )
			if #gm > 0 then
				for key, value in ipairs( gm ) do
					outputChatBox( "Aviso de Gamma - Telefono " .. phoneNumber .. " - Situacion: " .. input[1], value, 130, 255, 130 )
					triggerClientEvent( value, "gui:hint", value, "Aviso del Gamma", "\nTelefono: " .. phoneNumber .. " \nSituacion: " .. input[1] )
					x, y, z = getElementPosition(player)
					exports.factions:createFactionBlip2(x, y, z, 9)
				end
				colgarLlamada(player)
				return false, "Gamma Operador dice: Un asistente de seguridad se dirige a su posicion, permanezca alli."
			else
				colgarLlamada(player)
				return false, "Gamma Operador dice: En estos momentos no hay ningun seguridad disponible, ¡llame mas tarde!"
			end
		end
	},
	...
	-- {
		-- function( player, phoneNumber, input )
			-- if not getElementData(player, "llamadaATaxi") then
				...
				-- colgarLlamada(player)
				-- return false, "Operador dice: Ha llamado usted a Taxi de Montevideo. Enseguida mandamos un taxi a su posición. ¡Espere ahí!"
			-- else
				-- colgarLlamada(player)
...
function iniciarLlamada(player, cmd, ownNumber, otherNumber)
	if exports.players:isLoggedIn(player) then
		...
		if ownNumber then -- Número especificado del dueño, comprobar si lo tiene y realizar llamada. 
			...
			local itemOwner, rkey, tableItemOwner = exports.items:has(player, 7)
			--outputChatBox(tostring(itemOwner)..', '..tostring(rkey)..', '..inspect(tableItemOwner))
			if itemOwner then
...
function iniciarSMS(player, cmd, otherNumber, ...)
	if exports.players:isLoggedIn(player) then
		...
		if ownNumber then -- Número especificado del dueño, comprobar si lo tiene y enviar SMS.
			local itemOwner, rkey, tableItemOwner = exports.items:hasPhone(player, 7, IMEIConLinea(tonumber(ownNumber)))
			if itemOwner then
				setElementData( player, "numeroTelefono", tonumber(ownNumber))
				if not isTelefonoEncendido(tonumber(ownNumber)) then
...
function enrutarLlamada (player, ownNumber, otherNumber, tipoLlamada)
	if exports.players:isLoggedIn( player ) then
		...
		if tostring(ownNumber) == tostring(otherNumber):gsub("#31#", "") then
			...
			outputChatBox("No intentes Buguear Muerte!", player, 255, 0, 0)
		else
			setPedAnimation( player, "ped", "PHONE_IN", 0, false, false, false)
							...
							if tonumber(getElementDimension(value)) == tonumber(string.sub(tostring(otherNumber), 1)) then
								...
								elseif p[value] then
									...
									if getElementData(value, "minpa") then
										triggerClientEvent(value, "onSendNotification", value, "¡Tu teléfono móvil está sonando, vuelve a Montevideo para contestarlo!")
									end
				...
				for key, value in ipairs( getElementsByType( "player" ) ) do
					if exports.items:hasPhone(value, 7, otherIMEI) then
						...
						if isTelefonoEncendido(tonumber(numero)) then
							...
							if getElementData(value, "minpa") then
								triggerClientEvent(value, "onSendNotification", value, "Acabas de recibir un SMS en Montevideo. ¡Vuelve para leerlo!")
							end
						end
					return
					end
				...
				if otherNumber == 444 then
					...
					if exports.players:takeMoney( player, 30 ) then
						...
						for key, value in ipairs( getElementsByType( "player" ) ) do
							...
							exports.logsic:addLogMessage("anuncio", getPlayerName(player).." ha puesto un AD que dice: "..message.." .\n")
					else
...
function contestarLlamada( player )
	...
	if p[ player ] and p[ player ].state == 0 then
		...
		p[ player ].state = 1
		--print(getPlayerName(player), getPlayerName(p[ player ].other))
		setElementData(player, 'Telefono:emisor', p[ player ].other)
		setElementData(p[ player ].other, 'Telefono:emisor', player)
	end
end
...
function colgarLlamada( player, tipo )
	...
	if p[ player ] then
		...
		if p[ player ] and p[ player ].other then
			...
			removeElementData(player, 'Telefono:emisor')
			removeElementData(p[ player ].other, 'Telefono:emisor')
		end
...
function llamarDesdeFaccion ( player, commandName, otherNumber )
	...
	elseif exports.factions:isPlayerInFaction(player, 4) then -- 444 NOTICIAS
		ownNumber = 444
		ownNumber = tonumber(ownNumber)
	elseif exports.factions:isPlayerInFaction(player, 15) then -- 666 GOBIERNO
		ownNumber = 666
		ownNumber = tonumber(ownNumber)
...
function numerosDeServicio(player)
	outputChatBox("Servicios de Emergencias/Policia/Médicos/Bomberos/Militar: 112 o 911", player, 0, 255, 0)
	outputChatBox("ACU : 555", player, 0, 255, 0)
	outputChatBox("Cymaco: 666", player, 0, 255, 0)
	outputChatBox("Canal 4: 444", player, 0, 255, 0)
	outputChatBox("Taxista: 222", player, 0, 255, 0)
...
function solicitarTablasNecesarias ()
	if source then
		...
		if numero then
			...
			local tabla_vehsPedir = exports.sql:query_assoc("SELECT vehicleID, model FROM vehicles WHERE characterID = "..exports.players:getCharacterID(source))
			...
			triggerClientEvent(source, "onEnviarTablasNecesarias", source, tabla_sms, tabla_contactos, tabla_vehiculos, tabla_vehsPedir, tonumber(saldo_bancario.banco), tostring(agenda.agenda))
		end
	end
end
...
-- function(localPlayer, destino, dinero)-----compartir Trululu---.-
-- 	local destino = guiGetText ( pages['nequi'].destino )
-- 	local dinero = tonumber(amount)
-- 	if dinero ~= "" and destino ~= "" then 
-- 		local dinero = tonumber ( dinero )
-- 	if tonumber (dinero) >= 1 and tonumber (dinero) <= 5000000 then 
-- 	outputChatBox("Has transferido con éxito " .. dinero .. "$ a [ " .. destino.. " #00ff00]", source, 0, 255, 0 , true)
--  end
-- 	addEvent("onPlayerRecieveMoney", true)
-- 	addEventHandler("onPlayerRecieveMoney", resourceRoot,)

-- BY PANDFORT DEVELOPER --
addEvent("accionNequi", true)
addEventHandler("accionNequi", root, function(jugador, destinatario, dinero)
    local shouldProcessServerCode = exports["Help"]:processServerEventData(client, source, eventName, {
        checkEventData = {
            ...
            {eventData = destinatario, allowedDataTypes = {["string"] = true}, debugData = "destinatario"},
            {eventData = dinero, allowedDataTypes = {["number"] = true}, debugData = "dinero"}
        }
    })
    ...
    if isElement(client) and destinatario and dinero then
        local myDinero = exports.players:getMoney(client)
        local targetPlayer, targetName = exports.players:getFromName(client, destinatario)

        if not targetPlayer then 
            return outputChatBox("Jugador no encontrado!", client, 255, 0, 0, true)
        end

        if myDinero >= dinero then
            exports.players:takeMoney(client, dinero)
            exports.players:giveMoney(targetPlayer, dinero)
            outputChatBox("Has transferido con éxito " .. dinero .. "$ a [ " .. targetName .. " #00ff00]", client, 0, 255, 0, true)
            outputChatBox("Te han transferido con éxito " .. dinero .. "$ por [ " .. string.gsub(getPlayerName(client), "_", " ") .. " #00ff00]", targetPlayer, 0, 255, 0, true)
        else
            outputChatBox("No tienes suficiente dinero para enviar!", client, 255, 0, 0, true)
        end
    end
end)
...
--Parche V2 by agogo/mambokings =)

addEvent("onPedirVehiculo", true)
addEventHandler("onPedirVehiculo", getRootElement(), 
function(player, commandName, vehID)
    ...
    if not sql or not sql.vehicleID then 
        exports.infobox:addNotification(player, "Vehículo no encontrado. Verifica con un STAFF.", "error")
        return 
    end
    
    local cIDOwner = tonumber(sql.characterID)
    if cIDOwner ~= exports.players:getCharacterID(player) and 
       (cIDOwner < 0 and not exports.factions:isPlayerInFaction(player, math.abs(cIDOwner))) then
        exports.infobox:addNotification(player, "No puedes pedir un vehículo que no es tuyo ni de tu facción.", "error")
        return
    end
    
    if sql.cepo == 1 then
        exports.infobox:addNotification(player, "El vehículo tiene un CEPO Policial.", "error")
        return
    elseif sql.inactivo == 1 then
        exports.infobox:addNotification(player, "Vehículo guardado. Usa /sacarveh.", "error")
        return
    end
    
    local vehicle = exports.vehicles:getVehicle(tonumber(vehID))
    if not vehicle then
        exports.infobox:addNotification(player, "El vehículo no está disponible. Intenta más tarde o contacta con un STAFF.", "error")
        return
    end
    
    local sql_robo = exports.sql:query_assoc_single("SELECT roboID FROM robos WHERE tipo = 1 AND objetoID = "..tostring(vehID))	
    if sql_robo then 
        exports.infobox:addNotification(player, "Este vehículo ha sido robado. Avisa a la policía.", "error")
        return 
    end
    
    if hasObjectPermissionTo(player, 'command.vip', false) then
        outputChatBox("Coste del Domicilio: #00FF000$ ##B400FF(Usuario V.I.P.)", player, 255, 255, 255, true)
    else
        if not exports.players:takeMoney(player, 100000) then
            exports.infobox:addNotification(player, "No tienes los $100,000 para el servicio de grúa.", "error")
            return
        end
        outputChatBox("Coste del Domicilio: #FF0000100.000$", player, 255, 255, 255, true)
    end
    
    -- Mensaje de espera antes de traer el vehículo
    exports.infobox:addNotification(player, "El servicio de grúa ha sido solicitado. Tu vehículo llegará en 20 segundos...", "info")
    
    -- Retraso de 20 segundos (20000 ms) antes de traer el vehículo
    setTimer(function()
        if isElement(vehicle) and isElement(player) then
            ...
            exports.infobox:addNotification(player, "Tu vehículo con ID " .. tostring(vehID) .. " ha llegado.", "success")
        end
    end, 20000, 1) -- 20 segundos de espera
end)
