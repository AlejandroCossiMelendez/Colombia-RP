...
-- Limpieza de recursos DX
addEventHandler("onClientResourceStop", resourceRoot, function()
	if isElement(filled) then destroyElement(filled) end
	if isElement(not_filled) then destroyElement(not_filled) end
end)

--- @param tbl table
--- @param value any
--- @return number, any | nil
function table.findItem(tbl, value)
	if (not tbl) or (type(tbl) ~= "table") then
		return false
	end
	for key, val in pairs(tbl) do
		if table.compare(val, value) then
			return key, val
		end
	end
	return false
...
--- @param value any
--- @return number, any | nil
function table.findItemReverse(tbl, value)
	if (not tbl) or (type(tbl) ~= "table") then
		return false
	end
	for key = #tbl, 1, -1 do
		local val = tbl[key]
		if table.compare(value, val) then
			return key, val
		end
	end
	return false
...
function table.compare(tbl1, tbl2)
	if (not tbl1) or not tbl2 or (type(tbl1) ~= "table") or (type(tbl2) ~= "table") then
		return false
	end
	for key, value in pairs(tbl1) do
		if value ~= tbl2[key] or tbl2.item == 29 and value ~= tonumber(tbl2[key]) then
			return false
		end
	end
	return true
