...
addEventHandler( getResourceName( resource ) .. ":characters", localPlayer,
	function( chars, spawn, token, ip )
		...
		-- No mostrar interfaz si estamos en proceso de spawn directo
		local directSpawn = getElementData(localPlayer, "direct_spawn_process")
		if directSpawn then
			outputDebugString("[Players] Bloqueando interfaz characters desde event :characters")
			return -- No mostrar la interfaz de selección
		end
			...
			setTimer(
				function( )
					if characters then
						...
						-- No mostrar interfaz si está en proceso de spawn directo
						local directSpawn = getElementData(getLocalPlayer(), "direct_spawn_process")
						if not loggedIn and not directSpawn then
							exports.gui:show( 'characters', true, true, true )
						elseif directSpawn then
							outputDebugString("[Players] Bloqueando interfaz characters desde timer resource start")
						end
					else
						triggerServerEvent("onRequestLoginPanel",getLocalPlayer())
					end
