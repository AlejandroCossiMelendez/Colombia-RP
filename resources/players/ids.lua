...
function getFromName( player, targetName, ignoreLoggedOut )
	if targetName then
		...
		if #match == 1 then
			local loggedChecker = _G.isLoggedIn or exports.players and exports.players.isLoggedIn
			local isLogged = false
			if type(loggedChecker) == "function" then
				isLogged = loggedChecker( match[ 1 ] ) and true or false
			end
			if isLogged or ignoreLoggedOut then
