				...
				if player ~= other then
					...
					if getDistanceBetweenPoints3D( x, y, z, getElementPosition( other ) ) < 5 then
						...
						if tonumber(sql3.diferencia) < 5 then 
							exports.logsic:addLogMessage("/pagos_nuevos/"..getPlayerName(player):gsub("_", " "), "Cantidad $"..tostring(amount).." pagado a "..tostring(name)..".")
						...
						if exports.players:takeMoney( player, amount ) then
						   if not getElementData(other, "mecaduty") == true then
								...
								--exports.chat:enviarLogDiscord("transferencias", getPlayerName(player):gsub("_", " ") .. " ha pagado $" ..tostring(amount).." a " .. getPlayerName(other):gsub("_", " "))
...
addEventHandler("onPCUAceptada", getRootElement(), function()
    local client = source
    
    -- Validación de seguridad
    local shouldProcessServerCode = exports["Help"]:processServerEventData(client, source, "onPCUAceptada", {
        checkEventData = {
...
addEventHandler("onPCURechazada", getRootElement(), function()
    local client = source
    
    -- Validación de seguridad
    local shouldProcessServerCode = exports["Help"]:processServerEventData(client, source, "onPCURechazada", {
        checkEventData = {
            {eventData = source, equalTo = client, debugData = "source"}
        }
    })
		...
		if exports.players:isLoggedIn(value) and not getElementData(value, "ajail") and not getElementData(value, "tjail") then
			if getElementData(value, "sed") and getElementData(value, "sed") <= 0 then
				if (getElementHealth(value)-5) >= 2 then
					...
					triggerEvent("onSufrirDamageCapitalRP", value, value, 0, 61, true, true)
...
addEventHandler("onGestionarPeso", root, function(gordura, peso)
    local client = source
    
    -- Validación de seguridad
    local shouldProcessServerCode = exports["Help"]:processServerEventData(client, source, "onGestionarPeso", {
        checkEventData = {
            {eventData = source, equalTo = client, debugData = "source"},
            {eventData = gordura, allowedDataTypes = {["number"] = true}, debugData = "gordura"},
            {eventData = peso, allowedDataTypes = {["number"] = true}, debugData = "peso"}
        }
    })
	...
	function(cmd)
		...
		elseif (commandSpam[source] >= 8) then
			...
			setTimer(function(source)
				setElementData(source, "muTemp", nil)
