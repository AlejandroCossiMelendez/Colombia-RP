...
function isPlayerAtAutoescuela()
...
-- Evento para iniciar el test teórico desde el servidor
addEvent("startLicenseTest", true)
addEventHandler("startLicenseTest", localPlayer, function()
    createlicenseTestIntroWindow()
    showCursor(true)
end)

-- Evento para iniciar el test práctico desde el servidor
addEvent("initiateDrivingTest", true)
addEventHandler("initiateDrivingTest", localPlayer, function()
    initiateDrivingTest()
end)
             
function solicitarLicencia(cmd, licencia)
	outputChatBox("=== INFORMACIÓN DE AUTOESCUELA ===", 255, 255, 0)
	outputChatBox("Para obtener tu licencia de conducir:", 255, 255, 255)
	outputChatBox("1. Ve al marcador verde cerca de la playa de Los Santos", 0, 255, 0)
	outputChatBox("2. Entra al marcador para iniciar automáticamente el proceso", 0, 255, 0)
	outputChatBox("3. Necesitas $100,000 para el examen teórico", 255, 255, 0)
	outputChatBox("4. Necesitas $150,000 adicionales para el examen práctico", 255, 255, 0)
	outputChatBox("¡Ya no es necesario usar comandos, solo ve al marcador!", 0, 255, 255)
end
...
---------------------------------------
testRoute = {
    {1046.4560546875, -1712.3154296875, 13.149534225464},
    {1037.9560546875, -1644.1904296875, 13.15145111084},
    {1051.34765625, -1494.873046875, 13.155076026917},
    {1139.380859375, -1406.134765625, 13.263551712036},
    {1323.3935546875, -1406.5751953125, 13.094535827637},
    {1396.376953125, -1433.7275390625, 13.148297309875},
    {1446.0478515625, -1442.958984375, 13.144494056702},
    {1429.2255859375, -1578.70703125, 13.132100105286},
    {1427.5390625, -1720.5283203125, 13.151426315308},
    {1326.8193359375, -1731.1904296875, 13.151491165161},
    {1311.5, -1840.2646484375, 13.144690513611},
    {1185.2197265625, -1848.4912109375, 13.169486999512},
    {1182.25390625, -1722.4912109375, 13.275210380554},
    {1070.1240234375, -1708.890625, 13.153937339783}
}
...
function startDrivingTest(element)
	if element == getLocalPlayer() then
		...
		elseif getPlayerMoney(localPlayer) >= 150000 then
			destroyElement(blip)
			destroyElement(marker)
			outputChatBox("Se te han descontado 150.000 pesos como cuota del exámen.", 255, 194, 14)
			triggerServerEvent("payFeeCapitalRP", getLocalPlayer(), 150000)
