...
local cocina = {
    [2337] = true, -- Cocina 1 (ya estaba)
    [2529] = true, -- Cocina 2 (ya estaba)
    [2518] = true  -- Horno/Microondas (nuevo)
}

local electrodomesticos = {
    [1210] = true, -- Refrigerador 1 (nuevo)
    [1828] = true, -- Refrigerador 2 (nuevo)
    [2149] = true, -- Refrigerador 3 (nuevo)
    [2961] = true, -- Licuadora (nuevo)
    [2962] = true  -- Cafetera (nuevo)
}

-- Decoración
local decoracion = {
    [3461] = true, -- Lámpara de Pie (nuevo)
    [14872] = true, -- Lámpara Moderna (nuevo)
    [14873] = true, -- Lámpara Clásica (nuevo)
    [2717] = true, -- Reloj de Pared 1 (nuevo)
    [2718] = true, -- Reloj de Pared 2 (nuevo)
    [621] = true, -- Planta Pequeña (nuevo)
    [1271] = true, -- Planta Alta (nuevo)
    [1272] = true, -- Planta de Interior (nuevo)
    [1273] = true, -- Planta con Maceta (nuevo)
    [1274] = true, -- Planta Estilizada (nuevo)
    [11729] = true, -- Alfombra Elegante (nuevo)
    [11731] = true  -- Alfombra Clásica (nuevo)
}

-- Entretenimiento
local entretenimiento = {
    [2912] = true, -- Consola de Videojuegos (nuevo)
    [2226] = true, -- Equipo de Sonido 1 (nuevo)
    [2230] = true, -- Equipo de Sonido 2 (nuevo)
    [2588] = true, -- Proyector de Cine (nuevo)
    [2627] = true, -- Mesa de Billar Clásica (nuevo)
    [2628] = true, -- Mesa de Billar Moderna (nuevo)
    [2779] = true, -- Máquina Arcade 1 (nuevo)
    [2780] = true, -- Máquina Arcade 2 (nuevo)
    [2781] = true  -- Máquina Arcade 3 (nuevo)
}

-- Muebles de oficina
local oficina = {
    [1808] = true, -- Escritorio Elegante (nuevo)
    [2298] = true, -- Escritorio de Lujo (nuevo)
    [2338] = true, -- Escritorio Moderno (nuevo)
    [2339] = true, -- Escritorio Clásico (nuevo)
    [2356] = true, -- Librero 1 (nuevo)
    [2357] = true, -- Librero 2 (nuevo)
    [2358] = true, -- Librero 3 (nuevo)
    [2741] = true, -- Archivero 1 (nuevo)
    [2742] = true, -- Archivero 2 (nuevo)
    [2743] = true  -- Archivero 3 (nuevo)
}

-- Muebles para exteriores
local exteriores = {
    [1279] = true, -- Silla de Jardín 1 (nuevo)
    [1280] = true, -- Silla de Jardín 2 (nuevo)
    [1281] = true, -- Mesa de Jardín 1 (nuevo)
    [1282] = true, -- Mesa de Jardín 2 (nuevo)
    [14840] = true, -- Parrilla Clásica (nuevo)
    [14841] = true, -- Parrilla Moderna (nuevo)
    [14844] = true, -- Sombrilla 1 (nuevo)
    [14845] = true  -- Sombrilla 2 (nuevo)
}
...
addEventHandler( "onElementClicked", getRootElement(),
	function( button, state, player ) 
		...
		elseif button == "right" and state == "down" and muebleID and source and getElementDimension(player) == getElementData(player, "muebleintID") and not getElementData(player, "moviendoMueble") and not getElementData(player, "guiMueble") and not getElementData(source, "estado") then
...
function guardarMuebles(player)
	if exports.players:isLoggedIn(player) then
		...
		local int = exports.interiors:getInterior(getElementDimension(player))
		if (int and int.characterID == exports.players:getCharacterID(player)) or exports.items:has(player, 2, getElementDimension(player)) then
			local mueblesEliminadosCorrectamente, mueblesEliminadosFallidos = 0, 0
			for k, v in ipairs(exports.sql:query_assoc("SELECT muebleID, skin, extra FROM muebles WHERE dimension = "..tonumber(dimension))) do
				if (getElementData(muebles[v.muebleID].mueble, "estado") ~= 1) then
					...
					mueblesEliminadosCorrectamente = mueblesEliminadosCorrectamente + 1
				else
					mueblesEliminadosFallidos = mueblesEliminadosFallidos + 1
				end
			end
			return mueblesEliminadosCorrectamente, mueblesEliminadosFallidos
		end
	end
	return false
end
