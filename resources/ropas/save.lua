...
function guardRopas(charID, strClothes)
	exports.sql:query_free( "UPDATE characters SET clothes = '%s' WHERE characterID = " .. charID, strClothes )
	exports.sql:query_free( "UPDATE characters SET skin = 0 WHERE characterID = " .. charID )
end
...
function applyClothes (player)
	...
	if clothes then
		...
		for k, v in ipairs(clothes) do
			...
			if not textura then
				exports.logsic:addLogMessage("ropabug", getPlayerName(player).." - Código Ropa: "..tostring(v)..".")
			else
