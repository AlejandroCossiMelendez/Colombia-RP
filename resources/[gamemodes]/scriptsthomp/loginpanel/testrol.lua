...
function onTestRol()
	...
	dxDrawText('> Test de Rol Vendetta RP <',  sx/2 - 0.175*sx, sy/2 - 0.14*sy, 0.35*sx + (sx/2 - 0.175*sx),  0.3*sy + (sy/2 - 0.14*sy), tocolor(255,151,16,255), 1, font2, "center", "top", false, false,false)
    ...
    if selected == 1 then
   		dxDrawRectangle(sx * 0.3398 + mx, sy * 0.4922 + mx, sx * 0.0176 - mx*2, sy * 0.0221 - mx*2, tocolor(255,151,16,200), false)
    end
    ...
    if selected == 2 then
   		dxDrawRectangle(sx * 0.3398 + mx, sy * 0.5469 + mx, sx * 0.0176 - mx*2, sy * 0.0221 - mx*2, tocolor(255,151,16,200), false)
    end
    ...
    if selected == 3 then
   		dxDrawRectangle(sx * 0.3398 + mx, sy * 0.6042 + mx, sx * 0.0176 - mx*2, sy * 0.0221 - mx*2, tocolor(255,151,16,200), false)
    end

    dxDrawRoundedRectangle(sx * 0.4297 + p1, sy * 0.6523 + p1, sx * 0.1406 - p1*2, sy * 0.0352 - p1*2, tocolor(255,151,16,200), 12)
		...
		if current <= total then
			...
			if current > total then
				if correctas >= minimas then
					...
					notiTest('Pasaste el test de rol, guardando...',255,151,16)
