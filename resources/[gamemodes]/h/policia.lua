...
addCommandHandler( "duty",
	function( thePlayer, commandName )
		if exports.players:isLoggedIn( thePlayer ) then
			...
			if getElementDimension(thePlayer) == 1203 then
		...
		if exports.factions:isPlayerInFaction(player, 1) or exports.factions:isPlayerInFaction(player, 2) or exports.factions:isPlayerInFaction(player, 5) or exports.factions:isPlayerInFaction(player, 6) then
			...
			if x and y and z then
				if not getElementData(player, "dref") then
					if not aviso then
						exports.factions:sendMessageToFaction(1, "[Departamental] "..getPlayerName( player ):gsub( "_", " " ).." ha mandado su posición.", 87, 35, 100)
						exports.factions:sendMessageToFaction(2, "[Departamental] "..getPlayerName( player ):gsub( "_", " " ).." ha mandado su posición.", 87, 35, 100)
						exports.factions:sendMessageToFaction(5, "[Departamental] "..getPlayerName( player ):gsub( "_", " " ).." ha mandado su posición.", 87, 35, 100)
						exports.factions:sendMessageToFaction(6, "[Departamental] "..getPlayerName( player ):gsub( "_", " " ).." ha mandado su posición.", 87, 35, 100)
					end
					for key, value in ipairs( getElementsByType("player") ) do
					...
					if not aviso then
						exports.factions:sendMessageToFaction(1, "[Departamental] "..getPlayerName( player ):gsub( "_", " " ).." ha retirado su posición.", 87, 35, 100)
						exports.factions:sendMessageToFaction(2, "[Departamental] "..getPlayerName( player ):gsub( "_", " " ).." ha retirado su posición.", 87, 35, 100)
						exports.factions:sendMessageToFaction(5, "[Departamental] "..getPlayerName( player ):gsub( "_", " " ).." ha retirado su posición.", 87, 35, 100)
						exports.factions:sendMessageToFaction(6, "[Departamental] "..getPlayerName( player ):gsub( "_", " " ).." ha retirado su posición.", 87, 35, 100)
					end
...
function arrestar (player, commandName, otherPlayer, tiempo, ...)
	...
	if other and razon and tiempo and tonumber(tiempo) then
		if exports.factions:isPlayerInFaction(player, 1) then
			if (getElementDimension(player) ~= 1203) or (getElementDimension(other) ~= 1203) then outputChatBox("(( No puedes arrestar a alguien estando fuera de comisaria ))", player, 255, 0, 0) return end
			for k, v in ipairs(getElementsByType("player")) do
				if exports.factions:isPlayerInFaction(v, 1) then
			...
			if getElementDimension(other) == 1203 then
				setElementPosition(other, 227.4296875, 109.2060546875, 999.015625)
				setElementInterior(other, 10)
				setElementDimension(other, 1203)
			else
				...
				if exports.factions:isPlayerInFaction ( player, 1 ) then
					...
					if ( distance < 5) then
						...
						if getElementData(player, "enc1") then
							outputChatBox( "Agente Policial", target, 255, 150, 0 )
						else
							outputChatBox( "Policia Nacional De Red Country", target, 255, 150, 0 )
						end
...
function getArmamento (player, cmd, wid)
	...
	if not getElementDimension(player) == 1203 then outputChatBox("No estás en comisaria.", player, 255, 0, 0) return end
	...
	local rango = result.factionRank
	if not rango or not result or rango < 0 then outputChatBox("No tienes suficiente rango como para acceder a este comando.", player, 255, 0, 0) return end
...
function getCargadores (player, cmd, wid)
	if not exports.factions:isPlayerInFaction(player, 1) then outputChatBox("No eres policía.", player, 255, 0, 0) return end
	if getElementDimension(player) ~= 1203 then outputChatBox("No estás en comisaria.", player, 255, 0, 0) return end
	local result = exports.sql:query_assoc_single( "SELECT factionRank FROM character_to_factions WHERE factionID = 1 AND characterID = " .. exports.players:getCharacterID(player))
	local rango = result.factionRank
	if not rango or not result or rango < 0 then outputChatBox("No tienes suficiente rango como para acceder a este comando.", player, 255, 0, 0) return end
...
function giveUtiles (player, cmd, otherPlayer)
	if not exports.factions:isPlayerInFaction(player, 1) then outputChatBox("No eres policía.", player, 255, 0, 0) return end
	if not (getElementInterior(player) == 10) then outputChatBox("No estás en comisaria.", player, 255, 0, 0) return end
	if otherPlayer then
	local other, name = exports.players:getFromName(player, otherPlayer)
	if other then
		local result = exports.sql:query_assoc_single( "SELECT factionRank FROM character_to_factions WHERE factionID = 1 AND characterID = " .. exports.players:getCharacterID(player))
		...
		if (exports.factions:isPlayerInFaction(other, 1)) and (getElementInterior(other) == 10) then
	...
	if hasObjectPermissionTo(player, 'command.modchat', false) or exports.factions:isPlayerInFaction(player, 1) then
		if getElementData( otro, "ajail") and tonumber(getElementData( otro, "ajail")) > 0 then
			if getElementDimension(otro) == 0 then
				setElementPosition(otro,  -218.0400390625, 979.435546875, 19.505647659302)
...
function getChaleco (player)
	if not exports.factions:isPlayerInFaction(player, 1) then outputChatBox("No eres policía.", player, 255, 0, 0) return end
	if getElementDimension(player) ~= 0 then outputChatBox("No estás en comisaria.", player, 255, 0, 0) return end
...
function crearBlipsAlStartResource2()
	for k, v in ipairs(getElementsByType("vehicle")) do
		if exports.vehicles:getOwner(v) == -1 and markerParkPDC and not isElementWithinMarker(v, markerParkPDC) and getElementDimension(v) ~= 1203 then 
...
function crearBlipsAlStartPJ()
	if source and exports.factions:isPlayerInFaction(source, 1) or getElementData(source, "enc1") then
		for k, v in ipairs(getElementsByType("vehicle")) do
			local x, y, z = getElementPosition(v)
			if exports.vehicles:getOwner(v) == -1 and not isElementWithinMarker(v, markerParkPDC) and getElementDimension(v) ~= 1203 then 
