		...
		for index, resource in ipairs ( getResources ( ) ) do
			if getResourceState ( resource ) == "running" and getResourceName ( resource ) ~= getThisResource ( ) then
				if not restartResource ( resource ) then
					outputServerLog( "restartall: Ha fallado el reinicio de la resource '" .. getResourceName ( resource ) .. "' .Inténtalo manualmente y avisa a Trululu." )
				end
			end
		end
	...
	for index, resource in ipairs ( getResources ( ) ) do
		if getResourceState ( resource ) == "loaded" then
			if not startResource ( resource ) then
				outputServerLog( "startall: Ha fallado el inicio de la resource '" .. getResourceName ( resource ) .. "' . Inténtalo manualmente y avisa a Trululu." )
			end
		end
	end
