...
function tutorial()
	...
	triggerServerEvent('tengo:caca', resourceRoot, localPlayer, -1411.7103271484, -827.50524902344, 14.83465385437, {0,0})
end
...
function endCamera()
  ...
  triggerServerEvent( "players:trailerAceptado", getLocalPlayer( ), getLocalPlayer ( ) )
...
function camera()
  ...
  local text = 'Presiona el botón #00ff00ESPACIO#ffffff durante #00ff00'..second..'#ffffff segundos, para saltarte esta escena.'
  ...
  if kamera == 1 then
    x, y, z = interpolateBetween( -1735.80078125, 233.48828125, 340.10159301758,-1738.80078125, 236.48828125, 343.10159301758, (getTickCount()-samolot1)/15000, 'Linear')
    setCameraMatrix(x,y,z,2500,-55000,-5275)
    x1,y1,z1 = getElementPosition(veh)
    setElementPosition(veh,x1,y1-0.1,z1-0.01)
  elseif kamera == 2 then
    x, y, z = interpolateBetween(-1771.3837890625, -808.1875, 22.812757492065,-1768.8681640625, -1406.20703125, 20.146724700928, (getTickCount()-samolot2)/20000, 'Linear')
    setCameraMatrix(x,y,z,8500,-1000,12)
  elseif kamera == 3 then
    x, y, z = interpolateBetween(-1451.9052734375, -988.74609375, 16.394437789917,-1460.2041015625, -975.1142578125, 16.52285194397, (getTickCount()-samolot3)/10000, 'Linear')
    setCameraMatrix(x,y,z,-12500,35000,14)
  elseif kamera == 4 then
    x, y, z = interpolateBetween(-1384.7099609375, -850.544921875, 16.459173202515,-1403.5712890625, -835.5712890625, 15.468245506287, (getTickCount()-samolot4)/10000, 'Linear')
    setCameraMatrix(x,y,z,-40000,35000,16)
  end
  

  if y >= 235.48828125 and kamera == 1 then
    fadeCamera(false, 1.0, 0, 0, 0)
  end

  if y >= 235.88828125 and kamera == 1 then
    fadeCamera(true, 0.5)
    ...
    veh = createVehicle(519,-1735.6630859375, -807.00811767578, 15.871248626709,0,0,184)
  end

  if x >= -1771.3837890625 and kamera == 2 then
    local x,y,z = getElementPosition(veh)
    setElementPosition(veh,x,y-0.52,z)
  end
  
  if y <= -1350.20703125 and kamera == 2 then
    fadeCamera(false, 1.0, 0, 0, 0)
  end
  
  if y <= -1406.20703125 and kamera == 2 then
    fadeCamera(true, 0.5)
    destroyElement(veh)
    samolot3 = getTickCount()
    kamera = 3

    veh = createVehicle(519,-1463.0703125, -960.9150390625, 15.751598358154, 0, 0, 120)
    ...
    pilot = createPed(253,-1469.6038818359, -965.97265625, 14.828077316284)
    ochrona1 = createPed(229,-1466.2532958984, -963.6953125, 14.828077316284)
    ochrona2 = createPed(229,-1460.8597412109, -966.771484375, 14.828077316284)
    ped6 = createPed(0,-1465.2532958984, -963.6953125, 14.828077316284)
	
    setElementRotation(pilot,0,0,225)
    setElementRotation(ochrona1,0,0,215)
    setElementRotation(ochrona2,0,0,152)
    setElementRotation(ped6,0,0,215)
    setPedAnimation(ped6, 'ped', 'WALK_civi')
	addPedClothes ( ped6, "shirtbplaid", "shirtb", 0 )
  ...
  if y >= -975.1142578125 and kamera == 3 then
    destroyElement(veh)
    destroyElement(pilot)
    destroyElement(ochrona1)
    destroyElement(ochrona2)
    ...
    ped6 = createPed(0,-1400.7819824219, -838.18011474609, 14.832485198975)
	addPedClothes ( ped6, "shirtbplaid", "shirtb", 0 )
    setPedAnimation(ped6, 'ped', 'WALK_civi')
	setElementRotation(ped6,0,0,48)
    setTimer(fadeCamera,5800,1,false, 1.0, 0, 0, 0)
    setTimer(fadeCamera,6800,true,0.5)
    setTimer(destroyElement,6800,1,ped6)
  end
  
  -- -1465.1130371094, -831.56488037109
  
  if y == -835.5712890625 and kamera == 4 then
    endCamera()
  end
...
--]]
  ...
  if x >= 1669 and kamera == 2 then
    ...
    ped6 = createPed(26,1926.4916992188,-2309.0817871094,13.546875)
  ...
  if x >= 1921 and kamera == 3 then
    ...
    ped6 = createPed(26,1768.4836425781,-2439.8366699219,13.5546875)
    setPedAnimation(ped6, 'ped', 'WALK_civi')
