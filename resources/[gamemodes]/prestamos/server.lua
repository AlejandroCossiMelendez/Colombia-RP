...
function verMiPrestamo (player, comando, tipo)
	...
	if tienePrestamos(player, tonumber(tipo)) == true then
		if tienePrestamosPersonaje(player) == true then
			...
			outputChatBox("~~Datos de préstamos - Colombia Roleplay~~", player, 255, 255, 255)
			...
			outputChatBox("Cuota por Dia De Paga: #FF0000$"..tostring(sql.cuota), player, 255, 255, 255, true)
			if sql.tipo == 1 then
