...
addEvent( "onCharacterLogout", true )
...
local groups = {
	{ groupName = "Developers", groupID = 1, aclGroup = "Desarrollador", displayName = "Desarrollador", nametagColor = { 238, 130, 238 }, priority = 2 },
	...
	if p[ player ] and isLoggedIn( player ) then
		...
		if getOption( player, "staffduty" ) then
			for key, value in ipairs( groups ) do
				if type(aclGetGroup( value.aclGroup )) ~= 'boolean' and isObjectInACLGroup( "user." .. p[ player ].username, aclGetGroup( value.aclGroup ) ) and value.nametagColor then
							...
							if account then
								...
								if isGuestAccount( getPlayerAccount( player ) ) and not logIn( player, account, info.mtasalt ) then
									...
									if serial ~= sql.regSerial and tostring(serial) ~= tostring(sql.regSerial2) then
										...
										triggerEvent( "onCharacterLogoutBank", player)
...
local function showLoginScreen( player, screenX, screenY, token, ip )	
	...
	spawnPlayer( source, 74.407424926758, -279.44647216797, 1.7879505157471, 88.251800537109, 0, 0, 1 )
	...
	function( )
		...
		for key, value in ipairs( getElementsByType( "player" ) ) do
			...
			if p[ value ] and p[ value ].charID then
				...
				triggerEvent( "onCharacterLogoutBank", value)
	...
	function( )
		if source == client then
			...
			if p[ source ].charID then
				...
				triggerEvent( "onCharacterLogoutBank", source )
			...
			if tonumber( userID ) and tonumber( charID ) then
				...
				if char then
					...
					if char.nuevo == 0 then
						spawnPlayer( 1714.8740234375, -1911.94921875, 13.566590309143, 0, char.interior, char.dimension )
						fadeCamera( source, true )
						setCameraTarget( source, source )
						setCameraInterior( source, char.interior )
						setElementData(source, "characterID", tonumber(charID))
						
						triggerEvent( "onCharacterLoginBank", source)
					end 
					if char.nuevo == 1 then
						spawnPlayer( source,  87.28125, -279.64453125, 1.578125, 265.15081787109, 0, 0 )
						fadeCamera( source, true )
						setCameraTarget( source, source )
						setCameraInterior( source, 0 )
...
function getPlayerByCharID( characterID )
...
function getMoney( player, amount )
	return isLoggedIn( player ) and getPlayerMoney(player) or 0
end
...
function createCharacter( player, name, edad, genero, color, ropas, idCabeza)
	if player and p[ player ].userID then
		...
		elseif exports.sql:query_free( "INSERT INTO characters (characterName, userID, x, y, z, edad, genero, color, interior, dimension, rotation) VALUES ('%s', " .. p[ player ].userID .. ", 2216.27, 137.76, 26.48, "..tonumber(edad)..", "..tonumber(genero)..", "..tonumber(color)..", 0, 0, 270)", name ) then
			...
			--
			if genero == 1 then
				local chaR = exports.sql:query_assoc_single( "SELECT characterID FROM characters WHERE characterName = '%s'", name ) 
				triggerEvent('GuardarCabeza', player, chaR.characterID, idCabeza, genero)
				exports.ropas:guardRopas(chaR.characterID, ropas)
			end
			return true
		end
	end
