...
"¡Visita el Discord En https://discord.gg/7yWGVuTDG9", 
"Si observas algún bug, No Abuses!",  
"¡Los Staff No Somos Mecanicos, Ni Medicos!",
"Antes De Pedir Fix, Revisa Si Hay Mecanicos!",
"Recuerda: Cualquier Tipo de AntiRol Toma F12 Y Ticket En Discord!"}

function mensajeAleatorio()
	outputChatBox(anuncios[math.random(1, #anuncios)], root, 59, 163, 188)
end
setTimer(mensajeAleatorio, 20000, 1)
				...
				if other ~= player then
					...
					if vehicle then
						...
						if not teleported then
							...
							outputChatBox( "Has transportado a " .. name .. " hacia ti.", player, 140, 0, 75 )
				...
				if other ~= player then
					...
					if vehicle then
						...
						if not teleported then
							...
							outputChatBox( "Te has transportado a " .. name .. ".", player, 140, 0, 75 )
							outputChatBox( getPlayerName( player ):gsub( "_", " " ) .. " se ha transportado hacia ti.", other, 140, 0, 75 )
						end
					else	
		...
		if otherPlayer then
			...
			if other then
				if player and other then
					...
					if frozen then
						outputChatBox( "Has descongelado a " .. name .. ".", player, 140, 0, 75 )
						if player ~= other then
							outputChatBox( "Has sido descongelado por " .. getPlayerName( player ):gsub("_", " ") .. ".", other, 140, 0, 75 )
						end
					else
						outputChatBox( "Has congelado a " .. name .. ".", player, 140, 0, 75 )
						if player ~= other then
							outputChatBox( "Has sido congelado por " .. getPlayerName( player ):gsub("_", " ") .. ".", other, 140, 0, 75 )
						end
					end
			...
			if other then
				...
				if health < 1 then
					if triggerEvent("onSufrirAtaque", other, player, 0, 60, true) then
						outputChatBox( "Has matado a " .. name .. ".", player, 140, 0, 75 )
						outputChatBox( "El staff " .. getPlayerName(player):gsub("_", " ") .. " te ha matado.", other, 140, 0, 75 )
					end
				elseif setElementHealth( other, health ) then
					removeElementData(other, "muerto")
					outputChatBox( "Has cambiado la vida de " .. name .. " a " .. health .. ".", player, 140, 0, 75 )
					if getElementData(player, "enc") then
						outputChatBox( "Alguien te ha cambiado la vida a "..health.. ".", other, 140, 0, 75 )
					else
...
addEventHandler( "onPlayerQuit", root,
	...
	if reason == "Test fallido, conéctate para inténtarlo..." then return end
	if type == "Kicked" then
        ...
        if otherPlayer and skin then
            ...
            if other then
                ...
                if oldSkin == skin then
                    outputChatBox( name .. " ya estas utilizando esta skin.", player, 255, 255, 0 )
                elseif characterID and setElementModel( other, skin ) then
                    if exports.sql:query_free( "UPDATE characters SET skin = " .. skin .. " WHERE characterID = " .. characterID ) then
                        ...
                        outputChatBox( "Error al guardar Skin", player, 255, 0, 0 )
                        setElementModel( other, oldSkin )
                    end
                else
                    outputChatBox( "Skin " .. skin .. " es invalida", player, 255, 0, 0 )
                end
            end
        else
...
function darArma ( thePlayer, commandName, otro, weaponID, ammo )
	if hasObjectPermissionTo( thePlayer, "command.createvehicle", false ) then
		local other = exports.players:getFromName( thePlayer, otro )
		...
		if other then
			...
			outputChatBox( "/"..commandName.." [id jugador][id arma][municion]", thePlayer, 255, 255, 255 ) 
		end
	end
end
