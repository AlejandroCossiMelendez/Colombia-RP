...
function getPos (player)
	...
	outputChatBox("Tu rotación es: "..tostring(r1)..", "..tostring(r2)..", "..tostring(r3)..".", player, 0, 166, 214)
	outputChatBox("Tus coordenadas son: "..tostring(x)..", "..tostring(y)..", "..tostring(z)..".", player, 0, 166, 214)
	outputChatBox("Tu estás en: "..tostring(getZoneName(x, y, z, false))..", "..tostring(getZoneName(x, y, z, true))..".", player, 0, 166, 214)
	outputChatBox("Estas en el interior: "..tostring(getElementInterior(player)).." y dimension: "..tostring(getElementDimension(player))..".", player, 0, 166, 214)
end
...
function consultarMisHoras(player)
	if player and exports.players:isLoggedIn(player) then
		...
		if sql then
			outputChatBox("Llevas jugadas "..tostring(sql.horas).." horas en este personaje.", player, 0, 166, 214)
		end
	end
end
...
function toggleObjetos (player)
	...
	if getElementData(player, "tid") == true then
		outputChatBox("Has desactivado el modo para ver IDs de objetos de los mapas.", player, 0, 166, 214)
		setElementData(player, "tid", false)
	else
		outputChatBox("Has activado el modo para ver IDs de objetos de los mapas.", player, 0, 166, 214)
		outputChatBox("Pulsa en cualquier objeto para ver su ID y coordenadas.", player, 0, 166, 214)
...
function quitPlayer ( quitType )
	if source and quitType then
		...
		elseif quitType == "Kicked" then
			exports.chat:sendLocalOOC( source, "[Conexión] " .. getPlayerName(source):gsub("_", " ") .. " se ha desconectado forzosamente por un kick de un Dios." )
		elseif quitType == "Banned" then
			exports.chat:sendLocalOOC( source, "[Conexión] " .. getPlayerName(source):gsub("_", " ") .. " se ha desconectado forzosamente por un ban de un Dios." )
...
function ayudavips(player)
    ...
    outputChatBox("~~~~~~ Ayuda usuario VIP Vendetta ~~~~~~", player, 180, 0, 255)
...
function miID (player, cmd, otherPlayer)
	if exports.players:isLoggedIn(player) and not (otherPlayer) then
		...
		outputChatBox("También puedes usar /id [parte del nombre] para saber su ID.", player, 0, 166, 214)
	else
