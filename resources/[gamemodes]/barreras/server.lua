...
-- Puertas policia
barreraPol = createObject ( 968, 632.2998046875, -594.7998046875, 16.10000038147, 0, 90, 94.246215820313 )
markerPol = createMarker (631.71856689453, -591.20916748047, 16.333442687988, "cylinder", 6, 255, 0, 0, 0 )
esbarpol = 0
...
function toggleBarreraPol(element)
	...
	if exports.factions:isPlayerInFaction( player, 1 ) then
		if esbarpol == 0 then
			...
			moveObject ( barreraPol, 1200, 632.2998046875, -594.7998046875, 16.10000038147, 0, -90, 0)
			esbarpol = 1
			setTimer( function () 
			moveObject ( barreraPol, 1200, 632.2998046875, -594.7998046875, 16.10000038147, 0, 90, 0)
			end, 5000, 1, player)
