...
function solicitarReparto(player)
	...
	if exports.factions:isPlayerInFaction(player, 7) then
		...
		if (exports.factions:isPlayerInFaction(player, 7) and getDistanceBetweenPoints3D(334.8505859375, -60.27734375, 1.519495010376, x, y, z) > 5) then outputChatBox("(( Debes de estar en el Punto de Base para los Repartos ))", player, 255, 0, 0) return end
		local sql = exports.sql:query_assoc("SELECT * FROM interiors ORDER BY productos ASC")
		for k, v in ipairs(sql) do
			if v and v.interiorID and exports.interiors:isTienda(v.interiorID) == true and v.productos <= 250 and reparto == false and (v.interiorType == 0 or (v.interiorType == 2 and v.characterID > 0)) then
				reparto = true
	...
	if getElementType(vehicle) == "vehicle" and (getElementModel(vehicle) == 414) then
		...
		if source == marker2T[player] then
			if getElementData(player, "int.reparto") and getElementData(player, "int.reparto") > 0 then
				...
				if exports.factions:isPlayerInFaction(player, 7) then 
					outputChatBox("Has rellenado este local por $"..tostring(math.floor(0.075*getElementHealth(vehicle))).." correctamente.", player, 0, 255, 0)
				    exports.players:giveMoney(player, math.floor(0.075*getElementHealth(vehicle)))
...
addCommandHandler('shopsvacios',
	function(player)
		if hasObjectPermissionTo( player, "command.createshop", false ) or hasObjectPermissionTo( player, "command.deleteshop", false ) then
			local reparto = false
			local sql = exports.sql:query_assoc("SELECT * FROM interiors ORDER BY productos ASC")
			for k, v in ipairs(sql) do
				if v and v.interiorID and exports.interiors:isTienda(v.interiorID) == true and v.productos <= 250 and (v.interiorType == 0 or (v.interiorType == 2 and v.characterID > 0)) then
					reparto = true
					outputChatBox("El local '"..tostring(v.interiorName).."'("..v.interiorID..") necesita productos.", player, 0, 255, 0)
				end	
			end
			if reparto == false then
				outputChatBox("(( Actualmente no hay locales que requieran productos ))", player, 255, 0, 0) 
...
addCommandHandler('rellenarshops',
	function(player)
		if hasObjectPermissionTo( player, "command.createshop", false ) or hasObjectPermissionTo( player, "command.deleteshop", false ) then
			...
			for k, v in ipairs(sql) do
				if v and v.interiorID and exports.interiors:isTienda(v.interiorID) == true and v.productos <= 250 and (v.interiorType == 0 or (v.interiorType == 2 and v.characterID > 0)) then
					exports.interiors:giveProductos(tonumber(v.interiorID), 500)
				end	
			end
			outputChatBox("* Se abastacieron todas las tiendas.", player, 0, 255, 0) 
		end
	end
)