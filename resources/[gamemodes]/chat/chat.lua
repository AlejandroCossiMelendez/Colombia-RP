...
function me( source, message, prefix )
	if isElement( source ) and getElementType( source ) == "player" and type( message ) == "string" then
		if prefix then
			...
			localMessage( source, message, 255, 0, 128 )	
		else
	...
	function( thePlayer, commandName, ... )
		if exports.players:isLoggedIn( thePlayer ) then
			...
			if #message > 0 then
				...
				local resto = string.sub(message, 2)
				local mensaje = tostring(pletra..resto)
				localMessage( thePlayer, " *" .. mensaje .. " ((" .. getPlayerName( thePlayer ) .. "))", 0, 155, 125 )
			else
		...
		if exports.players:isLoggedIn( thePlayer ) and not isPedDead( thePlayer ) then
			if exports.factions:isPlayerInFaction ( thePlayer, 1) or exports.factions:isPlayerInFaction ( thePlayer, 2) then 
				...
				if #message > 0 and not getElementData(thePlayer, "nogui") == true then
					...
					local resto = string.sub(message, 2)
					local mensaje = tostring(pletra..resto)
					localizedMessage( thePlayer, " [" .. getPlayerName( thePlayer ) .. "] [Megáfono]: ", mensaje .. ( mensaje:sub( #mensaje ) ~= "." and mensaje:sub( #mensaje ) ~= "!" and mensaje:sub( #mensaje ) ~= "?" and "!" or "" ), 203, 50, 52, 130 )
				end
			end
		end
	end
			...
			if #message > 0 and not getElementData(thePlayer, "nogui") == true then
				...
				elseif getElementData(thePlayer, "qOOC") then
					...
					if getElementData(thePlayer, "account:gmduty") == true then
						localMessage( thePlayer, "[STAFF LocalOOC] ID: [" .. exports.players:getID( thePlayer ) .. "] " .. getPlayerName( thePlayer ) ..  ": (( " .. message .. " ))", 3, 255, 104 )
					else
						localMessage( thePlayer, "[LocalOOC] "  .. getPlayerName( thePlayer ) ..  ": (( " .. message .. " ))", 196, 255, 255 )
				...
				if #message > 0 and not getElementData(thePlayer, "nogui") == true then
					for k, v in ipairs(getElementsByType("player")) do
						--if hasObjectPermissionTo( v, "command.modchat", false ) then
							outputChatBox( "Global OOC ((" .. getPlayerName( thePlayer ) ..  ")): " .. message .. " ", v, 59, 131, 189 )
						--else
							--outputChatBox( "[Administración] " .. message .. " ", v, 0, 255, 153 )
						--end
					end
				else
					outputChatBox( "Syntax: /" .. commandName .. " [Canal OOC]", thePlayer, 255, 255, 255 )
	...
	function( thePlayer, commandName, ... )
		if exports.players:isLoggedIn( thePlayer ) then
			...
			if #message > 0 and not getElementData(thePlayer, "nogui") == true then
				...
				local groups = exports.players:getGroups( thePlayer )
				if groups and #groups >= 1 then
					message = "[#eb2de0StaffChat#cdf213]#f41128 "..groups[1].displayName .. " " .. message
				end
		...
		if exports.players:isLoggedIn( thePlayer ) then
			if exports.factions:isPlayerInFactionType( thePlayer, 1 ) then		
				...
				if #message > 0 then
					...
					local resto = string.sub(message, 2)
					local mensaje = tostring(pletra..resto)
					localizedMessage( thePlayer, " [Departamental] " .. getPlayerName( thePlayer ) ..  " dice: ", mensaje, 115, 194, 251, players )
					localizedMessage( thePlayer, " " .. getPlayerName( thePlayer ) .. " dice por su radio: ", mensaje, 230, 230, 230, false, 115, 194, 251 )
				else
...
local function pm( player, target, message, enc )
    ...
    if getElementData(target, "minpa") then
		triggerClientEvent(target, "onSendNotification", target, getPlayerName(player).." te ha enviado un PM en Vendetta RP. ¡Vuelve para leerlo!")
	end
	if enc then
...
function tirarDado( thePlayer, commandName )
	if exports.players:isLoggedIn( thePlayer ) and not isPedDead( thePlayer ) then
		me( thePlayer, "tira un dado y sale " .. math.random( 6 ) .. ".", "Dados" )
	end
end
			...
			if #message > 0 and not getElementData(thePlayer, "nogui") == true then
				...
				for key, value in ipairs( getElementsByType( "player" ) ) do
					if hasObjectPermissionTo( value, "command.modchat", false ) then
						outputChatBox( "[Anuncio Caracol] ((["..getPlayerName(thePlayer):gsub("_"," ").."])) " .. message, value, 106, 255, 255 )
					else
						outputChatBox( "[Anuncio Caracol] " .. message .. ".", value, 106, 255, 255 )
					end				
				end
				...
				if #message > 0 and not getElementData(thePlayer, "nogui") == true then
					...
					for k, v in ipairs(getElementsByType("player")) do
						if not getElementData(v, "radio") or getElementData(v, "radio") ~= true then
							outputChatBox("[Español] [Caracol] "..getPlayerName(thePlayer):gsub("_", " ").." dice: "..tostring(message), v, 62, 184, 255)
						end
					end
				else
		...
		if exports.players:isLoggedIn( thePlayer ) then
              if getElementData( thePlayer, "entrevistado" ) == true then
				...
				if #message > 0 then
					...
					for k, v in ipairs(getElementsByType("player")) do
						if not getElementData(v, "radio") or getElementData(v, "radio") ~= true then
							outputChatBox("[Entrevistado] [Español] [Caracol] "..getPlayerName(thePlayer):gsub("_", " ").." dice: "..tostring(message), v, 113, 237, 31)
						end
					end
				else
...
function(player)
	if exports.factions:isPlayerInFaction(player, 4) then
		...
		outputChatBox("No perteneces a la facción de Caracol.", player, 255, 0, 0)
	end
end
)
