...
function comprarPiezas(player, cmd, cantida)
	if exports.factions:isPlayerInFaction(player, 3) then
		...
		local cantidad = math.abs(math.floor(cantida))
		if exports.factions:isPlayerInFaction(player, 3) and not isElementInRange(player, 245.43496704102, -120.67167663574, 1.5679507255554, 5) then outputChatBox("(( No estás en el taller. ))", player, 255, 0, 0) return end
		if cantidad and cantidad > 0 then
			if exports.players:takeMoney(player, tonumber(cantidad*15)) then
				if exports.factions:isPlayerInFaction(player, 3) then
					exports.factions:giveFactionPresupuesto(3, tonumber(cantidad*15))
				end
				...
				outputChatBox ("Has comprado "..tostring (cantidad).." piezas por $"..tostring(cantidad*15)..",recuerda trabajar y atender clientes.", player, 0, 255, 0)
...
addCommandHandler("comprarpiezas 20", confirmarCompraPiezas)

local VendedorPed = createPed(50, 754.7001953125, -3865.7377929688, 1.3406250476837, 173.14990234375)
setElementInterior( comprarPed, 0)
setElementDimension( comprarPed, 0)
...
function(button,state, hit)
	...
	local source = hit
	if (source == comprarPed) or (source == comprarPiezas) then
