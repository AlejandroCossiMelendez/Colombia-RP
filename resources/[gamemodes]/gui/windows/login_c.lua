...
windows.login =
{
	...
	{
		type = "label",
		text = "Necesitas una cuenta para jugar en ExoticRP\nSi tienes una cuenta, incia sesión.",
		alignX = "center",
	},
	{
		type = "label",
...
function iniTest()
   ...
   ------------ TUTORIAL QUIZ SECTION - SCRIPTED BY PETER GIBBONS (AKA JASON MOORE), ADAPTED BY CHAMBERLAIN  AND EDITED BY Edvis20 FOR ExoticRP COUNTY ROLEPLAY --------------                               
questions = { 
...
function createlicenseTestIntroWindow()	
	...
	guiIntroWindow = guiCreateWindow ( X , Y , Width , Height , "Test de rol. ExoticRP." , false )
	
	guiCreateStaticImage (0.35, 0.1, 0.3, 0.2, "banner.png", true, guiIntroWindow)
	
	guiIntroLabel1 = guiCreateLabel(0, 0.3,1, 0.5, [[Vas a realizar el test de rol de ExoticRP.
	Son 20 preguntas tipo test, pudiendo fallar 2. Te recomendamos ir a https://exoticrp.com/foro]], true, guiIntroWindow)	
...
function createTestFinishWindow()
	...
	if(score >= passPercent) then
		...
		guiFinalFailLabel = guiCreateLabel(0, 0.3, 1, 0.1, "No has aprobado. Te recomendamos que vayas a exoticrp.com/foro", true, guiFinishWindow)
