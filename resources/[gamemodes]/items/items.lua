		...
		if source == client then
			if exports.players:isLoggedIn( source ) then
				...
				if item then
					...
					if id == 5552555 then
						local t = fromJSON(value)
						_toys[t[2]] = _toys[t[2]] or {}
						_toys[t[2]][source] = _toys[t[2]][source] or {}

						local check = true

						if isElement(_toys[t[2]][source][1]) then
							exports.bone_attach:detachElementFromBone(_toys[t[2]][source][1])
							destroyElement(_toys[t[2]][source][1])

							if _toys[t[2]][source][2] == t[1] then
								check = false
								exports.chat:me(source, 'se quita su '..name:sub(1, #name-2))
							end
						end

						if check then
							_toys[t[2]][source][1] = createObject(t[1], getElementPosition( source ))
							_toys[t[2]][source][2] = t[1]
							setObjectScale(_toys[t[2]][source][1], (t[9] or 1))
							setElementCollisionsEnabled( _toys[t[2]][source][1], false )

							setTimer(
								function(t, player)
									exports.bone_attach:attachElementToBone(_toys[t[2]][player][1], player, 1, t[3], t[4], t[5], t[6], t[7], t[8])
								end,
							100,1, t, source)

							exports.chat:me(source, 'se coloca un/a '..name:sub(1, #name-2))
						end
					elseif id == 5551555 then
						...
						local t = fromJSON(value)
						addPedClothes(source,t[1],t[2],t[3])
						if color == 1 then -- Blanco
							triggerClientEvent("onSolicitarRopaBlanco", source)
						end
						exports.chat:me(source, 'se cambia de prenda')
...
setTimer(
	function()
		for i,v in pairs(_toys) do
			for player, av in pairs(v) do
				if isElement(av[1]) then
					if getElementDimension( player ) ~= getElementDimension( av[1] ) then
						setElementDimension( av[1], getElementDimension( player ))
					end
					if getElementInterior( player ) ~= getElementInterior( av[1] ) then
						setElementInterior( av[1], getElementInterior( player ))
					end
				end
			end
		end
				...
				if item then
					...
					if id == 27 then
						...
						if id == 5552555 then
							...
							if isElement(_toys[t[2]][source][1]) and _toys[t[2]][source][2] == t[1] then
								exports.bone_attach:detachElementFromBone(_toys[t[2]][source][1])
								destroyElement(_toys[t[2]][source][1])
							end
						elseif id == 9 then
