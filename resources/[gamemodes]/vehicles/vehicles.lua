...
function createVehicleFromConce( player, model2 )
	if isElement( player ) then
		...
		if characterID then
		    local vehicle = createVehicle(tonumber(model2), 217.00253295898, -188.16064453125, 1.5778739452362, 216.72265625, -198.6201171875, 1.5778739452362)
...
addCommandHandler( { "respawnvehicles", "regenerarvehicles", "rvs" },
    ...
    ,160000, 1
		...
		outputChatBox( "[Administración]: Todos los vehiculos reaparecerán en 2 Minutos.", root, 0, 255, 153 )
	end,
	true
)
...
function (player, commandName, otherPlayer)
    ...
    if getDistanceBetweenPoints3D( x, y, z, getElementPosition( otro ) ) < 5 then
        if vehicle then
            exports.chat:me( player, "le muestra los papeles del coche a " .. nombre .. "." )
            outputChatBox ( "----------Papeles--------", otro, 0, 255, 0 )
            if data then
                local name = exports.players:getCharacterName( data.characterID )
                local faction = data.characterID
                local factionName = exports.factions:getFactionName ( -faction ) or "Nadie"
                if data.characterID > 0 then
                    outputChatBox ( "Dueño: ".. name .. "", otro, 0, 255, 120 )
                else
                    outputChatBox ( "Dueño: "..factionName, otro, 0, 255, 120 )
                end
            end
            outputChatBox ( "Modelo: " .. modelo .. " ", otro, 0, 255, 120 )
            outputChatBox ( "Matricula: " .. matricula, otro, 0, 255, 120)
            outputChatBox ( "------------------------------------", otro, 0, 255, 0 )
        end
    end
end
)