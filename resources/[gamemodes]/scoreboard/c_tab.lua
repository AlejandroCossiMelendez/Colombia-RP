...
local SCOREBOARD_DISABLED_CONTROLS	= { "next_weapon",	-- Controls that are disabled when the scoreboard is showing
										...
										"radio_next",
										"radio_previous" }
local SCOREBOARD_TOGGLE_TIME		= 10				-- Time in miliseconds to make the scoreboard (dis)appear
...
local SCOREBOARD_BACKGROUND			= { 0, 0, 0, 190 }			-- RGBA color for the background
...
drawScoreboard = function ()
	...
	local serverName = getElementData ( g_scoreboardDummy, "serverName" ) or "Star Colombia Roleplay"
	...
	local strPlayerCount = "Jugadores: " .. tostring(#g_players) .. "/" .. tostring(maxPlayers) .. ""
	...
	-- Draw the headers
	drawRowBounded ( "ID", "Nombres ", "Nivel", "Ping",
	...
	local renderEntry = function ( player )
		...
		-- Render it!
		top = top
		drawRowBounded ( playerID, playerName, playerLevel, playerPing, colors, "default-bold", top )
	end
