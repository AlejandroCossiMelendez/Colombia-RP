----------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------
local marker = createMarker(1308.716796875, -750.7451171875, 79.335899353027 -1, "cylinder", 1.50, 0, 255, 0, 100)
local int = 0
local dim = 0
...
function start(playerSource, qntd)
	...
	if getPolicias() < 0 then
		exports.a_infobox:addBox(playerSource, "Para fabricar una Colt deben haber al menos 2 policías en servicio", "error")
		return
	end
	...
	local materiales = {
		{id = 58, nombre = "Cuerpo de Colt-45"},
		{id = 59, nombre = "Cargador de Colt-45"},
		{id = 60, nombre = "Cañon de Colt-45"}
	}
	...
	-- 🔹 Fabricación segura
	local tiempoFabricacion = 45000 -- 2 minutos
	...
	exports.a_infobox:addBox(playerSource, "Fabricando Colt-45, espera 45 segundos...", "info")
	...
	setTimer(function()
		...
		local armaDada = exports.items:give(playerSource, 29, "22", "Colt-45", 1)
		if armaDada then
			exports.a_infobox:addBox(playerSource, "Has fabricado una Colt-45", "success")
			exports.chat:me(playerSource, "ha fabricado una Colt-45.")
		else
			exports.a_infobox:addBox(playerSource, "Error al entregar Colt-45, contacta admin.", "error")
		end
		...
		for _, v in ipairs(getElementsByType("player")) do
			if exports.factions:isPlayerInFaction(v, 1) or exports.factions:isPlayerInFaction(v, 4)  then
				...
				outputChatBox("(("..name..")) Emergencia - Alarma Fabrica de Armas Mansion Madd Dog", v, 130, 255, 130)
