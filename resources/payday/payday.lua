...
function darPayday (player)
	local h, m = getTime()
	outputChatBox("#2EFF00<< Fondo $ Solidario >>",player, 122, 173, 255, true)
	...
	if exports.players:isLoggedIn(player) then
		...
		if exports.factions:takeFactionPresupuesto(6, 100000) then
			outputChatBox("-Sueldo del Estado: #00FF00100.000 pesos", player, 255, 255, 255, true)
			darDinero(player, 100000)
			g = g + 100000
			exports.sql:query_free("UPDATE ajustes SET valor = valor + "..tostring(15000).. " WHERE ajusteID = 3")
		else
			outputChatBox("-Sueldo del Estado: #FF00000 pesos", player, 255, 255, 255, true)
		end
		local tieneFac = 0
		for k2, v2 in ipairs(exports.factions:getPlayerFactions(player)) do
			if tonumber(v2) < 100 then
				...
				if exports.factions:takeFactionPresupuesto(tonumber(v2), sueldo) then
					if tieneFac == 0 then
						...
						outputChatBox("-Sueldo de "..tostring(exports.factions:getFactionName(v2))..": #00FF00"..tostring(sueldo).." pesos", player, 255, 255, 255, true)
						g = g + sueldo
						...
						if v2 < 11 then
							...
							exports.logsic:addLogMessage("notificaciones", getPlayerName(player).. " DEBE DE SER EXPULSADO DE SUS F3")
						end
					end
				end
			end
			...
			--[[for k3, v3 in ipairs(exports.interiors:getInteriorsTipo(player, 1)) do
				outputChatBox("-Impuestos de "..tostring(v3.interiorName)..": #FF0000"..tostring(v3.interiorPrice/450).." pesos", player, 255, 255, 255, true)
		...
		if tieneLocales > 1 then outputChatBox("Te recordamos que no está permitido tener más de 1 local por PJ.", player, 255, 0, 0) exports.logsic:addLogMessage("localespd", getPlayerName(player).. " DEBE DE SER SANCIONADO, HA RECAUDADO DINERO DE " .. tostring(tieneLocales) .. " LOCALES") end
		tieneLocales = 0
	...
	if player and mid and v and v > 0 then
		...
		if getPlayerMoney(player) >= v then	
			if exports.players:takeMoney(player, tonumber(v)) then
				...
				outputChatBox ( "-Multa ID "..v5.ind..": #FF0000"..tostring(v).." pesos", player, 255, 255, 255, true )
			...
			if not quitarDinero(player, tonumber(v)) then
				...
				if exports.players:takeMoney(player, dinero) then
					...
					outputChatBox ( "-Multa ID "..v5.ind..": #FF0000"..tostring(dinero).." pesos", player, 255, 255, 255, true )
...
function pagarSalarioSemanal()
    local time = getRealTime()
    
    -- Verificar si es Viernes (weekday == 5)
    if time.weekday == 0 then
        outputDebugString("[PAYDAY] Hoy es viernes, iniciando el pago semanal...")

        local faccionesPermitidas = {1, 10} -- ID de Policía y Ejército

        for _, player in ipairs(getElementsByType("player")) do
            if exports.players:isLoggedIn(player) then
                local characterID = exports.players:getCharacterID(player)

                -- Obtener TODAS las facciones del jugador
                local faccionesJugador = exports.sql:query_assoc("SELECT factionID, factionRank FROM character_to_factions WHERE characterID = " .. characterID)

                outputDebugString("[PAYDAY] Verificando jugador: " .. getPlayerName(player))

                local esElegible = false
                local salario = 0 -- Salario predeterminado en caso de error
                local rangoMaximo = 1
                local faccionSeleccionada = nil

                if faccionesJugador and type(faccionesJugador) == "table" then
                    -- Recorrer todas las facciones del jugador
                    for _, data in ipairs(faccionesJugador) do
                        local faccion = tonumber(data.factionID)
                        local rango = tonumber(data.factionRank)

                        if table.includes(faccionesPermitidas, faccion) then
                            outputDebugString("[DEBUG] " .. getPlayerName(player) .. " tiene Rango " .. rango .. " en Facción " .. faccion)

                            -- Si este rango es mayor al guardado, lo reemplazamos
                            if rango > rangoMaximo then
                                rangoMaximo = rango
                ...
                -- Verificar si el jugador pertenece a una facción permitida
                if esElegible and faccionSeleccionada then
                    -- Consultar salario según el rango en la BD
                    local datosRango = exports.sql:query_assoc_single("SELECT salary FROM faction_ranks WHERE factionID = " .. faccionSeleccionada .. " AND factionRankID = " .. rangoMaximo)

                    if datosRango and datosRango.salary then
                        salario = tonumber(datosRango.salary)
                    else
                        salario = 30000 -- Salario mínimo si no se encuentra en la BD
                    end

                    outputDebugString("[PAYDAY] " .. getPlayerName(player) .. " recibirá $" .. salario .. " por su rango " .. rangoMaximo .. " en la facción " .. faccionSeleccionada)

                    -- Revisar si ya recibió el pago semanal
                    local lastPayday = exports.sql:query_assoc_single("SELECT last_weekly_pay FROM characters WHERE characterID = " .. characterID)

                    if not lastPayday or tonumber(lastPayday.last_weekly_pay) ~= time.yearday then
                        -- Dar el pago
                        if salario > 0 and exports.players:giveMoney(player, salario) then
                            -- Guardar en la base de datos que ya recibió el pago esta semana
                            exports.sql:query_free("UPDATE characters SET last_weekly_pay = " .. time.yearday .. " WHERE characterID = " .. characterID)

                            -- Mensaje de confirmación
                            outputChatBox("[SALARIO SEMANAL] Has recibido $" .. salario .. " por tu trabajo en la facción.", player, 0, 255, 0)

                            -- Log en consola
                            outputDebugString("[PAYDAY] Pago de $" .. salario .. " realizado a " .. getPlayerName(player) .. " (Rango: " .. tostring(rangoMaximo) .. ")")

                            -- Marcar al jugador como notificado
                            jugadoresNotificados[player] = true
                        else
                            outputChatBox("[SALARIO SEMANAL] Error al procesar tu pago. Contacta a un administrador.", player, 255, 0, 0)
                        end
                    else
                        if not jugadoresNotificados[player] then
                            outputChatBox("[SALARIO SEMANAL] Ya has recibido tu pago semanal. Usa /statuspayday para verificar el estado.", player, 255, 165, 0)
                            jugadoresNotificados[player] = true
                        end
                    end
                end
        ...
        -- Reiniciar la tabla de notificaciones si ya no es viernes
        jugadoresNotificados = {}
    end
end

-- Función auxiliar para verificar si un valor está en una tabla
...
-- Comando para verificar el estado del pago semanal
function verificarEstadoPayday(player)
    local characterID = exports.players:getCharacterID(player)
    local time = getRealTime()

    -- Obtener el último pago desde la BD
    local lastPayday = exports.sql:query_assoc_single("SELECT last_weekly_pay FROM characters WHERE characterID = " .. characterID)

    if lastPayday and tonumber(lastPayday.last_weekly_pay) == time.yearday then
        outputChatBox("[STATUS PAYDAY] Ya has recibido tu pago semanal.", player, 255, 165, 0)
    else
        outputChatBox("[STATUS PAYDAY] Aún NO has recibido tu pago semanal.", player, 0, 255, 0)
    end
end
addCommandHandler("statuspayday", verificarEstadoPayday)

-- Ejecutar pago semanal automáticamente cada 30 minutos sin detenerse - 1800000
setTimer(pagarSalarioSemanal, 1800000, 0)]]--
