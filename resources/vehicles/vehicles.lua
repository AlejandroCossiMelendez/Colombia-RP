    ...
    function(player, commandName, ...)
        ...
        if model then
            ...
            if vehicle then
                ...
                local vehicleID, error = exports.sql:query_insertid(
                    "INSERT INTO vehicles (model, posX, posY, posZ, rotX, rotY, rotZ, color1, color2, color3, respawnPosX, respawnPosY, respawnPosZ, respawnRotX, respawnRotY, respawnRotZ, interior, dimension, respawnInterior, respawnDimension, numberplate) VALUES (" ..
                    table.concat({model, x, y, z, 0, 0, rz, "'".. c1 .."'", "'".. c2 .."'", "'".. c3 .."'", x, y, z, 0, 0, rz, getElementInterior(player), getElementDimension(player), getElementInterior(player), getElementDimension(player), "'".. getVehiclePlateText(vehicle) .."'"}, ", ") .. ")")

                if vehicleID then
                    -- Guardar datos del vehículo
                    vehicleIDs[vehicleID] = vehicle
                    vehicles[vehicle] = {
                        vehicleID = vehicleID, respawnInterior = getElementInterior(player), 
                        respawnDimension = getElementDimension(player), characterID = 0, 
                        engineState = false, tintedWindows = false, fuel = 100
                    }				
                    ...
                    local logMessage = {
                        ...
                        description = 
                            ...
                            "> **Modelo:** " .. getVehicleName(vehicle) .. "\n" ..
                            "> **ID del Vehículo:** " .. vehicleID .. "\n" ..
                            "> **Placa:** " .. getVehiclePlateText(vehicle) .. "\n" ..
                            "> **Posición:** X: " .. x .. ", Y: " .. y .. ", Z: " .. z,
                        color = 3447003,
                        footer = {
                            text = "Registro de creación de vehículos",
                    ...
                    exports.discord_webhooks:sendToURL(
                        "https://discord.com/api/webhooks/1336529483972345949/1vCIQhdajhrNK5WCb_dOyUYurl8-XmGWqyO7RoHGYSRvKj1pJflysm8QhWH4ZQY95kPR",
                        logMessage
                    )
                else
...
function CreateConceVehicle(player, model, position, rotation)
	...
	if not rotation or type(rotation) ~= "number" then
		return false
	end
	local character_id = exports["players"]:getCharacterID(player)
	if not character_id then
		return false
	end
	local element = createVehicle(model, position[1], position[2], position[3], 0, 0, rotation)
	if not element then
		return false
	end
	local interior = getElementInterior(player)
	local dimension = getElementDimension(player)
	local r, g, b, r2, g2, b2 = getVehicleColor(element)
	local luz1, luz2, luz3 = getVehicleHeadLightColor(element)
	local c1 = RGB2HEX(r or 255, g or 255, b or 255)
	local c2 = RGB2HEX(r2 or 255, g2 or 255, b2 or 255)
	local c3 = RGB2HEX(luz1 or 255, luz2 or 255, luz3 or 255)
	local vehicle_id, error = exports.sql:query_insertid(
		"INSERT INTO vehicles (model, posX, posY, posZ, rotX, rotY, rotZ, color1, color2, color3, respawnPosX, respawnPosY, respawnPosZ, respawnRotX, respawnRotY, respawnRotZ, interior, dimension, respawnInterior, respawnDimension, numberplate, characterID) VALUES ("
			.. table.concat({
				model,
				position[1],
				position[2],
	...
	vehicleIDs[vehicle_id] = element
	vehicles[element] = {
		vehicleID = vehicle_id,
		respawnInterior = interior,
	...
	setVehicleColor(element, r, g, b, r2, g2, b2)
	setVehicleHeadLightColor(element, luz1, luz2, luz3)
	setVehicleRespawnPosition(element, position[1], position[2], position[3], 0, 0, rotation)
	...
	setElementData(element, "fuel", 100)
	setElementData(element, "km", 0)
	setElementData(element, "fasemotor", 0)
	setElementData(element, "fasefrenos", 0)
	setElementData(element, "marchas", 0)
	setElementData(element, "idveh", vehicle_id)
	setElementData(element, "idowner", character_id)
	setVehiclePlateText(element, vehicle_id)
	exports["vehicles_auxiliar"]:saveColors(element)
	exports["items"]:give(player, 1, vehicle_id)
	return element, vehicle_id
end
...
function createVehicleFromConce( player, model2 )
	if isElement( player ) then
		...
		if characterID then
		    local vehicle = createVehicle(tonumber(model2), 115.637, -158.157, 1.46788, 359.83, 0.258179, 91.2744)
				...
				if ( vehicleID >= 0 and not hasObjectPermissionTo( player, "command.createvehicle", false ) ) or ( vehicleID < 0 and not hasObjectPermissionTo( player, "command.temporaryvehicle", false ) ) then
					...
					if vehicle then
						if vehicleID < 0 or exports.sql:query_free( "DELETE FROM vehicles WHERE vehicleID = " .. vehicleID ) then
							...
							if vehicleID > 0 then
								exports.logsic:addLogMessage("delveh", "Vehículo ID "..vehicleID.. ", borrado por "..getPlayerName(player)..".\n")
							end
...
addCommandHandler("sacarvehfaccion", function(player)
    -- Verificar si el jugador pertenece a una facción válida
    local factionID = nil
    for i = 1, 19 do
    ...
    if not factionID then
        exports.infobox:addNotification(player, "No perteneces a ninguna facción.", "error")
        return
    end

    -- Obtener los vehículos de la facción usando query_assoc
    local query = exports.sql:query_assoc("SELECT vehicleID FROM vehicles WHERE characterID = " .. factionID)

    -- Verificar si la consulta falló
    if not query then
        outputDebugString("Error en la consulta SQL de vehículos de facción", 1)
        exports.infobox:addNotification(player, "Error en la base de datos. Contacta a un administrador.", "error")
        return
    end
    ...
    for _, row in ipairs(query) do
        table.insert(vehicles, tonumber(row.vehicleID))
    end

    -- Si no hay vehículos disponibles
    if #vehicles == 0 then
        exports.infobox:addNotification(player, "Tu facción no tiene vehículos asignados.", "error")
        return
    end

    -- Mostrar lista de vehículos
    outputChatBox("🚗 Vehículos disponibles para tu facción:", player, 0, 255, 0)
    for _, vehID in ipairs(vehicles) do
        outputChatBox(" - ID: "..vehID.." (Usa /pedirvehf "..vehID.." para solicitarlo)", player, 255, 255, 255)
    end
end)

-- Comando para pedir el vehículo
addCommandHandler("pedirvehf", function(player, _, vehID)
    vehID = tonumber(vehID)
    if not vehID then
        exports.infobox:addNotification(player, "Uso: /pedirvehf [ID del vehículo]", "info")
        return
    end

    -- Obtener la facción del jugador
    local factionID = nil
    for i = 1, 19 do
        if exports.factions:isPlayerInFaction(player, i) then
            factionID = -i
    ...
    -- Verificar si el vehículo pertenece a la facción del jugador
    ...
    if not sql or not sql.vehicleID then
        exports.infobox:addNotification(player, "Este vehículo no pertenece a tu facción.", "error")
        return
    end
    ...
    if not vehicle then
        exports.infobox:addNotification(player, "El vehículo no está disponible. Intenta más tarde.", "error")
        return
    end

    -- Verificar si alguien está usando el vehículo
    if getVehicleOccupant(vehicle) then
        exports.infobox:addNotification(player, "El vehículo ya está siendo utilizado por otro jugador.", "error")
        return
    end

    -- Mensaje de espera
    exports.infobox:addNotification(player, "El servicio de grúa ha sido solicitado. El vehículo llegará en 20 segundos...", "info")

    -- Esperar 20 segundos antes de mover el vehículo
    setTimer(function()
        if isElement(vehicle) and isElement(player) then
            -- Verificar nuevamente si alguien se subió mientras esperaba
            if getVehicleOccupant(vehicle) then
                exports.infobox:addNotification(player, "El vehículo fue ocupado mientras esperabas. No se movió.", "error")
                return
            end
            ...
            setElementDimension(vehicle, getElementDimension(player))
            setElementInterior(vehicle, getElementInterior(player))
            exports.infobox:addNotification(player, "El vehículo con ID " .. tostring(vehID) .. " ha llegado.", "success")
        end
					...
					if data.vehicleID < 0 or exports.sql:query_free( "UPDATE vehicles SET engineState = 1 - engineState WHERE vehicleID = " .. data.vehicleID ) then
						if exports.items:has ( player, 1, data.vehicleID ) or data.vehicleID < 0 or ( data.characterID < 0 and exports.factions:isPlayerInFaction( player, -data.characterID ) ) or ( models [ getElementModel ( vehicle ) ] ) or getElementData(player,"account:gmduty") == true or isVehiculoRobado(vehicle) == true then
							if math.floor( getElementHealth( vehicle ) + 0.5 ) > 359 then
                                ...
                                do
                                    local res = getResourceFromName("carrobomba")
                                    if res and getResourceState(res) == "running" then
                                        local ok, ret = pcall(call, res, "attemptEngineStart", player, vehicle)
			...
			if vehicle then
				...
				if other and vehicleID then
					if data.vehicleID < 0 then 
						...
						-- Log en Discord
						local logMessage = {
							title = "Asignación de Propietario de Vehículo",
							description = 
								"> **Administrador:** " .. getPlayerName(player):gsub("_", " ") .. "\n" ..
								"> **Nuevo Propietario:** " .. getPlayerName(other):gsub("_", " ") .. "\n" ..
								"> **Vehículo:** " .. getVehicleName(vehicle) .. "\n" ..
								"> **ID del Vehículo:** " .. vehicleID,
							color = 3447003,
							footer = {
								text = "Registro de cambios de propietario",
								icon_url = "https://imgur.com/Fbx9o6X"
							},
							timestamp = "now"
						}
						...
						exports.discord_webhooks:sendToURL(
							"https://discord.com/api/webhooks/1336518303988056179/lOQljdKgO-pGezr2qlhYexOCYExtGTI5O4nQRsz36Tx3Lshwhr0zYi9HbUJwArjAdIMt",
							logMessage
						)
					end
				end
...
addEventHandler("onElementClicked", resourceRoot, function(button, state, player)
	if button == "left" and state == "up" then
		local x, y, z = getElementPosition(player)
		local ox, oy, oz = getElementPosition(source)
		if
			getDistanceBetweenPoints3D(x, y, z, ox, oy, oz) <= 8
			and getElementDimension(player) == getElementDimension(source)
		then
			if getElementData(player, "mid") then
				outputChatBox(
					"Hemos detectado que has estado abriendo el maletero de un coche. Usa /dmal.",
					player,
					255,
					0,
					0
			...
			if
				getElementType(source) == "vehicle"
				and not getElementData(player, "mid")
				and not isPedInVehicle(player)
			then
				...
				local characterID = tonumber(characterID)
				if not getElementData(source, "idowner") then
					outputChatBox("Se ha producido un error. Contacta con un staff.", player, 255, 0, 0)
					return
				end
				local sql = exports.sql:query_assoc(
					"SELECT `index`, item, value, value2, name FROM maleteros WHERE vehicleID = " .. vehicleID
				)
				if
					exports.items:has(player, 1, vehicleID)
					or vehicleID < 0
					or (characterID < 0 and exports.factions:isPlayerInFaction(player, -characterID))
					or getElementData(player, "account:gmduty") == true
				then -- Revisar esta linea, puede haber ciertos errores.
					-- Tiene llave. ¿Es PD?
...
function desbugMaletero(player)
	if getElementData(player, "mid") then
		outputChatBox(
			"Has sido desbugeado, lo sentimos. Ya puedes abrir de nuevo cualquier maletero.",
			player,
			255,
			0,
			0
...
function toggleCepo(player, commandName, id, ...)
	if exports.factions:isPlayerInFaction(player, 1) or exports.factions:isPlayerInFaction(player, 11) then
...
addEvent( "onVehicles > sexWithDogs", true )
addEventHandler( "onVehicles > sexWithDogs", root,
	function ( a )
		if a then
			reloadVehicle( a )
		end
...
function createVehicleFromKit(player, nivea)
    if isElement(player) then
        local characterID = exports.players:getCharacterID(player)
        if characterID then
            local vehicle = createVehicle(tonumber(nivea), 1689.1, -2296.96, 13.0164, 1.27991, 359.94, 127.084)
            if vehicle then
                local model = getElementModel(vehicle)
                local x, y, z = getElementPosition(vehicle)
                local rx, ry, rz = getVehicleRotation(vehicle)
                ...
                local c2 = RGB2HEX(r2 or 255, g2 or 255, b2 or 255)
                local c3 = RGB2HEX(luz1 or 255, luz2 or 255, luz3 or 255)
                local query = "INSERT INTO vehicles (model, posX, posY, posZ, rotX, rotY, rotZ, color1, color2, color3, respawnPosX, respawnPosY, respawnPosZ, respawnRotX, respawnRotY, respawnRotZ, interior, dimension, respawnInterior, respawnDimension, characterID) VALUES (" ..
                    table.concat({model, x, y, z, rx, ry, rz, "'" .. c1 .. "'", "'" .. c2 .. "'", "'" .. c3 .. "'", x, y, z, rx, ry, rz, interior, dimension, interior, dimension, characterID}, ", ") .. ")"
                outputDebugString("Query: " .. query)
                local vehicleID, error = exports.sql:query_insertid(query)
                if vehicleID then
                    local newVehicle = createVehicle(model, x, y, z, rx, ry, rz, getVehiclePlateText(vehicle))
                    if newVehicle then
                        setElementFrozen(newVehicle, true)
                        ...
                        setVehicleColor(newVehicle, r, g, b, r2, g2, b2)
                        setVehicleHeadLightColor(newVehicle, luz1, luz2, luz3)
                        setVehicleRespawnPosition(newVehicle, x, y, z, rx, ry, rz)
                        setElementInterior(newVehicle, interior)
                        setElementDimension(newVehicle, dimension)
                        ...
                        setElementData(newVehicle, "idveh", vehicleID)
                        setElementData(newVehicle, "idowner", characterID)
                        setVehiclePlateText(newVehicle, tostring(vehicleID))
                        exports.vehicles_auxiliar:saveColors(newVehicle)
                        exports.items:give(player, 1, vehicleID) 
                        return newVehicle, vehicleID
                    end
                else
                    outputChatBox("Error al guardar el vehículo en la base de datos.", player, 255, 0, 0)
                    outputDebugString("Database error: " .. tostring(error)) -- Muestra el error específico en el log
                end
            else
                outputChatBox("Error al crear el vehículo.", player, 255, 0, 0)
            end
        end
    end
end
addEvent("generarkit", true)
addEventHandler("generarkit", root, createVehicleFromKit)



function darKitInicio(player, model2, posX, posY, posZ, rotX, rotY, rotZ)
	if isElement(player) then
		local characterID = exports.players:getCharacterID(player)
		if characterID then
		    -- Crear el vehículo en la posición y rotación especificadas
		    local vehicle = createVehicle(tonumber(model2), posX, posY, posZ, rotX, rotY, rotZ)
			local model = getElementModel(vehicle)
			local x, y, z = getElementPosition(vehicle)
			local rx, ry, rz = getVehicleRotation(vehicle)
			local interior = getElementInterior(vehicle)
			...
			local vehicleID, error = exports.sql:query_insertid("INSERT INTO vehicles (model, posX, posY, posZ, rotX, rotY, rotZ, color1, color2, color3, respawnPosX, respawnPosY, respawnPosZ, respawnRotX, respawnRotY, respawnRotZ, interior, dimension, respawnInterior, respawnDimension, numberplate, characterID) VALUES (" .. table.concat({model, x, y, z, rx, ry, rz, "'".. c1 .."'", "'".. c2 .."'", "'".. c3 .."'", x, y, z, rx, ry, rz, getElementInterior(player), getElementDimension(player), getElementInterior(player), getElementDimension(player), "'".. getVehiclePlateText(vehicle) .."'", characterID}, ", ") .. ")")
			...
			if vehicleID then
				...
				exports.sql:query_free("UPDATE vehicles SET characterID = " .. tonumber(characterID) .. " WHERE vehicleID = " .. vehicleID)
				exports.items:give(player, 1, vehicleID)
