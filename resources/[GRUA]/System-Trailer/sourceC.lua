	...
	function ()
											...
											if tx then
												if i == 4 then
													if holder == localPlayer then
														--outputChatBox("¡El cable no puede pasar aquí!", 255, 255, 255, true)
													    exports.Scripts_dxInfo:dxInfoAddBox("¡El cable no puede pasar aquí!", "info")
													end
		...
		if isElement(myTowTruck) then
			...
			if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) > 20 then
				--outputChatBox("El cable no llega tan lejos!", 255, 0, 0, true)
				exports.Scripts_dxInfo:dxInfoAddBox("El cable no llega tan lejos!", "info")
				...
				if state ~= "up" then
					...
					for k, v in pairs(keys) do
						if v == key then
							...
							if press then
								--outputChatBox("¡Empuje la rampa primero!", 255, 255, 255, true)
							    exports.Scripts_dxInfo:dxInfoAddBox("Empuje la rampa primero!", "info")
							end
						end
					end
				end
						...
						if not rampInAnim[veh] then
							if key == "cardown" then
								if getElementData(veh, "packerState") == "down" then
									...
									--outputChatBox("Suelte la rampa primero!", 255, 255, 255, true)
								    exports.Scripts_dxInfo:dxInfoAddBox("Suelte la rampa primero!", "info")
								end
									...
									if getVehicleType(clickedElement) == "Automobile" then
										...
										if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 5 then
											...
											if isInsideColShape(detectorColShape, x, y, z) and dist <= 2.5 then
												...
												--outputChatBox("¡El coche no está detrás del portacoches!", 255, 255, 255, true)
											    exports.Scripts_dxInfo:dxInfoAddBox("¡El coche no está detrás del portacoches!", "info")

											end
										else
											--outputChatBox("¡Estás demasiado lejos del vehículo!", 255, 255, 255, true)
										    exports.Scripts_dxInfo:dxInfoAddBox("¡Estás demasiado lejos del vehículo!", "info")
										end
									else
										--outputChatBox("Solo puedes reservar autos.", 255, 255, 255, true)
									    exports.Scripts_dxInfo:dxInfoAddBox("Solo puedes reservar autos.", "info")
									end
								end
							end
						end
