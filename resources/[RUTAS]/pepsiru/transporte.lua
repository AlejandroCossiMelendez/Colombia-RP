...
local vehicles = { "Benson" } -- vehículo definido para uso del job..
local max_earnings = 555700
local delay = 10
...
local function isJobVehicle( vehicle )
	return vehicle and vehicles[ getElementModel( vehicle ) ] and exports.vehicles:getOwner( vehicle ) == -6 or false
end
		...
		if c then
			if p[ player ].route[ p[ player ].checkpoint ] then
				if p[ player ].route[ p[ player ].checkpoint ].stop and earnings and earnings > 0 then
					...
					exports.factions:giveFactionPresupuesto( 6, 7500 )
				end
			...
			if earnings then
				exports.players:giveMoney( player, earnings )
				exports.factions:giveFactionPresupuesto( 6, 3500 )
			end
...
function crearParadasAuto()
	...
	for i = 1,2 do
		...
		for k, v in ipairs(rout) do
			local myMarker = createMarker(v.x, v.y, v.z-1, "cylinder", 6.0, 295,165,0,0)
...
addEventHandler( "onVehicleEnter", root,
	function( player, seat )
		if seat == 0 and isJobVehicle( source ) and exports.factions:isPlayerInFaction(player, 6) then
				...
				if not p[ source ].route[ p[ source ].checkpoint ].stop or getDistanceBetweenPoints2D( p[ source ].route[ p[ source ].checkpoint ].x, p[ source ].route[ p[ source ].checkpoint ].y, getElementPosition( vehicle ) ) < 10 then
					...
					if health > 350 then
						...
						if getElementData(source, "lastCPBus") == true then
							outputChatBox("Has finalizado tu ruta correctamente. Baja y sube del Camion para empezar de nuevo.", source, 0, 295, 0)
