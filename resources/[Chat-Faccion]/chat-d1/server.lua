function mensajePolicial(source, commandName, ...)
    ...
    if exports.factions:isPlayerInFaction(source, 15) then
        if mensajeAnuncio ~= "" and mensajeAnuncio ~= " " then
            for k, v in ipairs(getElementsByType("player")) do
                outputChatBox("#FFFFFF[LEGAL] [#FF1111Tiendas D1#FFFFFF]: #FFFFFF"..mensajeAnuncio, v, 255, 255, 255, true)
            end
        else
...
addCommandHandler("d1c", mensajePolicial)