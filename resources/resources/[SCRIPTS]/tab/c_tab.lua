...
local SCOREBOARD_SERVER_NAME_COLOR	= { 255, 245, 0, 255 }		-- RGBA color for the server name text
local SCOREBOARD_PLAYERCOUNT_COLOR	= { 255, 245, 0, 255 }	-- RGBA color for the server player count text
local SCOREBOARD_BACKGROUND			= { 0, 0, 0, 50 }			-- RGBA color for the background
...
local SCOREBOARD_SCROLL_BACKGROUND	= { 44, 44, 44, 100 }		-- RGBA color for the scroll background
local SCOREBOARD_SCROLL_FOREGROUND	= { 109, 109, 109, 160 }		-- RGBA color for the scroll foreground
...
drawBackground = function ()
	...
	if g_currentHeight > SCOREBOARD_HEADER_HEIGHT then
		...
		dxDrawImage ( SCOREBOARD_X - 3, SCOREBOARD_Y - 117,
					  SCOREBOARD_WIDTH + 6, SCOREBOARD_HEIGHT +121,
...
drawScoreboard = function ()
	...
	dxDrawFramedText ( serverName, left, top, right, bottom,
	...
	local strPlayerCount = "Ciudadanos: " .. tostring(#g_players) .. "/" .. tostring(maxPlayers) .. " (" .. math.floor(usagePercent + 0.5) .. "%)"
	...
	if left < right then
		dxDrawFramedText ( strPlayerCount, left, top, right, bottom,
					SCOREBOARD_PLAYERCOUNT_COLOR, 1, "default-bold", "left", "top",
					true, false, SCOREBOARD_POSTGUI )
	end
	...
	-- Draw the headers
	drawRowBounded ( "ID", "Nombre IC", "Ping",
