...
function enviarDineroNequi( jugador, destinatario, dinero )
	if isElement( jugador ) and destinatario and dinero then
		local myDinero = exports.players:getMoney( jugador ) -- obtenemos el dinero del jugador
		local semenDeCaballo, semenDePerro = exports.players:getFromName( jugador, destinatario ); -- obteemos el otro ugador
		if not semenDeCaballo then return outputChatBox( "Jugador no encontrado!", jugador, 255, 0, 0, true ) end
		if myDinero >= dinero then
			exports.players:takeMoney( jugador, dinero )
			exports.players:giveMoney( semenDeCaballo, dinero )
			outputChatBox("Has transferido con éxito " .. dinero .. "$ a [ " .. semenDePerro.. " #00ff00]", jugador, 0, 255, 0, true)
			outputChatBox("Te han transferido con éxito " .. dinero .. "$ por [ " .. string.gsub( getPlayerName( jugador ), "_", " " ).. " #00ff00]", semenDeCaballo, 0, 255, 0, true)
		else
			outputChatBox( "No tienes suficiente dinero para enviar!", jugador, 255, 0, 0, true )
		end
	end
end
addEvent( "accionNequi", true )
addEventHandler( "accionNequi", root, enviarDineroNequi )
...
function( player, commandName, vehID)
	...
	if exports.players:isLoggedIn(player) then
		...
		if not sql or not sql.vehicleID then 
		    exports.infobox:addNotification(player, "Vehiculo no encontrado, verifica con un STAFF", "error")
			return 
		end
		...
		if cIDOwner ~= exports.players:getCharacterID(player) and (cIDOwner < 0 and not exports.factions:isPlayerInFaction(player, math.abs(cIDOwner))) then
			outputChatBox("No puedes pedir un vehículo que no es ni tuyo ni de tu empresa.", player, 255, 0, 0)
			return
		end
		if sql.cepo == 1 then
		    exports.infobox:addNotification(player, "El vehiculo tiene un CEPO Policial", "error")
			return
		elseif sql.inactivo == 1 then
		    exports.infobox:addNotification(player, "Vehiculo Guardado, usa /sacarveh.", "error")
			return
		end
		local vehicle = exports.vehicles:getVehicle(tonumber(vehID))
		if not vehicle then
		    exports.infobox:addNotification(player, "El vehiculo no está disponible actualmente. Intenta de nuevo más tarde o contacta a un STAFF.", "error")
		    return
		end
		local sql_robo = exports.sql:query_assoc_single("SELECT roboID from robos WHERE tipo = 1 and objetoID = "..tostring(vehID))	
		if sql_robo then 
		    outputChatBox("Este vehículo ha sido robado. Avisa a la policía por si lo han recuperado.", player, 255, 0, 0)
		    return 
		end
		if hasObjectPermissionTo(player, 'command.vip', false) then
			...
			if exports.players:takeMoney(player, 100000) then
				outputChatBox("Coste del Domicilio: #FF0000100.000$", player, 255, 255, 255, true)
			else
			    exports.infobox:addNotification(player, "No tienes los 100,000 que cuesta el servicio de grua", "error")
				return
			end
		end
		...
		exports.infobox:addNotification(player, "Se ha traido el vehiculo con ID " ..tostring(vehID).."", "success")
	end
end)
