function mensajePolicial(source, commandName, ...)
    ...
    if exports.factions:isPlayerInFaction(source, 54) then
        if mensajeAnuncio ~= "" and mensajeAnuncio ~= " " then
            for k, v in ipairs(getElementsByType("player")) do
                outputChatBox("#FFFFFF[ LEGAL ] #FFFFFFAnuncios [#FFFFFFIngenio#FFA500SanCarlos] : #FFFFFF"..mensajeAnuncio, v, 255, 255, 255, true)
            end
        else
...
addCommandHandler("igscs", mensajePolicial)