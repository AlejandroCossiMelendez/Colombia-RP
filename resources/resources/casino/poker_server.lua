				...
				if pokerCurPlayers <= 5 then
					...
					for i = 1, 6 do
						if not _G["pokerPlayer"..i] then
							...
							sendMessageToAllPokerPlayers ( getPlayerName(hit).." se ha unido al juego.", 200, 200, 0 )
					...
					outputChatBox ( "Todos los sitios están ocupados, espera por favor.", hit, 125, 0, 0 )
				end
			end
		end
	end
...
function takePlayerChips ( player, amount )
	...
	if _G["pokerPlayer"..player.."Money"] < 0 then
		...
		sendMessageToAllPokerPlayers ( "¡"..getPlayerName(_G["pokerPlayer"..player]).." se ha puesto all in!", 0, 0, 170 )
	end
end
...
function roundCheck()
	...
	if pokerCurPlayers > 1 then
		curPokerPot = 0
		sendMessageToAllPokerPlayers ( "Crupier: ¡Nueva ronda!", 200, 200, 0 )
		...
		sendMessageToAllPokerPlayers ( "Crupier: Ciega grande: "..getPlayerName(_G["pokerPlayer"..curPokerBlindPlayer])..", Ciega pequeña: "..getPlayerName(_G["pokerPlayer"..smallBlindPlayer])..".", 200, 200, 0 )
		cardsInUse = nil
		cardsInUse = {}
		pokerFirstLoop = false
...
function playerTurnCheck_func ( player )
	...
	sendMessageToAllPokerPlayers ( getPlayerName(player).." pasa su turno.", 200, 0, 0 )
...
function playerTurnCall_func ( player )
	...
	local id = getPlayerPokerID ( player )
	sendMessageToAllPokerPlayers ( getPlayerName(player).." iguala la apuesta a "..getHighestCall()..".", 200, 0, 0 )
...
function playerTurnRaise_func ( amount )
	...
	local player = source
	sendMessageToAllPokerPlayers ( getPlayerName(player).." sube la apuesta a "..amount..".", 200, 0, 0 )
	local id = getPlayerPokerID ( player )
...
function playerTurnFold_func ()
	...
	local player = source
	local id = getPlayerPokerID ( player )
	sendMessageToAllPokerPlayers ( getPlayerName(player).." se retira.", 200, 0, 0 )
...
function nextRoundPlayer ()
	...
	if counter == 0 then
		sendMessageToAllPokerPlayers ( "Ronda finalizada.", 0, 125, 0 )
		resetPokerValues()
	elseif counter == 1 then
		sendMessageToAllPokerPlayers ( "Ronda finalizada. Ha ganado: "..getPlayerName(_G["pokerPlayer"..pokerCPlayerFolder]).." (los demás se retiraron)", 0, 125, 0 )
...
function setPlayerLeavingPoker_func ( player, id )
	...
	if isElement ( player ) then
		...
		sendMessageToAllPokerPlayers ( getPlayerName(player).." ha dejado el juego.", 200, 200, 0 )
...
function nextPlayer ()
	...
	outputChatBox ( "Apuesta actual: "..getHighestCall()..", tu apuesta: ".._G["pokerPlayer"..curPokerPlayer.."Bet"], _G["pokerPlayer"..curPokerPlayer], 150, 150, 25 )
	if isTimer ( timeoutTimer ) then
		killTimer ( timeoutTimer )
	end
...
function finalCheck ()
	...
	if curWinners == 1 then
		...
		if _G["pokerPlayer"..curWinner.."Money"] <= 0 then
			...
			sendMessageToAllPokerPlayers ( winnername.." ha ganado con "..value..". ( Mano: "..h1..", "..h2.." ), "..(_G["pokerPlayer"..curWinner.."TotalBet"] * 2).." fichas.", 0, 125, 0 )
			if curPokerPot > 0 then
				...
				sendMessageToAllPokerPlayers ( "Las restantes fichas ("..curPokerPot..") serán repartidas.", 0, 125, 0 )
				for i = 1, 6 do
					if _G["pokerPlayer"..i.."In"] then
						if i == curWinner then
						else
			...
			sendMessageToAllPokerPlayers ( winnername.." ha ganado con "..value..". ( Mano: "..h1..", "..h2.." ), "..curPokerPot.." fichas.", 0, 125, 0 )
		...
		sendMessageToAllPokerPlayers ( "Repartido entre: "..winnernames..", "..splot.." fichas para cada uno.", 0, 125, 0 )
		for i, key in ipairs ( curWinnersList ) do
