...
--QUATRO RODAS!!!
--==CARRO==-- 
if getElementModel(v) == 602 then --ID do Veiculo que deseja setar a hand
	...
	setVehicleHandling(v, "tractionLoss", 2.1110)
	setVehicleHandling(v, "tractionBias", 1.147)
	setVehicleHandling(v, "numberOfGears", 5)
	setVehicleHandling(v, "maxVelocity", 375) --Velocidade Máxima
	setVehicleHandling(v, "engineAcceleration", 100) -- Aceleração do Veículo
	setVehicleHandling(v, "engineInertia", 15)
	setVehicleHandling(v, "driveType", "awd")
	setVehicleHandling(v, "engineType", "petrol")
	setVehicleHandling(v, "brakeDeceleration", 45) --Força do freio
...
if getElementModel(v) == 00 then --ID do Veiculo que deseja setar a hand
	...
	setVehicleHandling(v, "maxVelocity", 220) --Velocidade Máxima
	setVehicleHandling(v, "engineAcceleration", 80) -- Aceleração do Veículo
	...
	setVehicleHandling(v, "tractionLoss", 2.1215)
	setVehicleHandling(v, "tractionBias", 1.123)
	setVehicleHandling(v, "centerOfMass", { 0, 0.15, -0.3 } )
