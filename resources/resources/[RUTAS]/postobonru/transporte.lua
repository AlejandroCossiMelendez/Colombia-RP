...
local vehicles = { "Remington" } -- vehículo definido para uso del job..
local max_earnings = 878000
local delay = 10
...
local function isJobVehicle( vehicle )
	return vehicle and vehicles[ getElementModel( vehicle ) ] and exports.vehicles:getOwner( vehicle ) == -19 or false
end
		...
		if c then
			if p[ player ].route[ p[ player ].checkpoint ] then
				if p[ player ].route[ p[ player ].checkpoint ].stop and earnings and earnings > 0 then
					...
					exports.factions:giveFactionPresupuesto( 19, 3500 )
				end
...
addEventHandler( "onVehicleEnter", root,
	function( player, seat )
		if seat == 0 and isJobVehicle( source ) and exports.factions:isPlayerInFaction(player, 19) then
				...
				if not p[ source ].route[ p[ source ].checkpoint ].stop or getDistanceBetweenPoints2D( p[ source ].route[ p[ source ].checkpoint ].x, p[ source ].route[ p[ source ].checkpoint ].y, getElementPosition( vehicle ) ) < 10 then
					...
					if health > 350 then
						...
						if getElementData(source, "lastCPBus") == true then
							outputChatBox("Has finalizado tu ruta correctamente. Baja y sube del Camion para empezar de nuevo.", source, 0, 255, 0)
