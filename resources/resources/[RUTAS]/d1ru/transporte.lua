...
local vehicles = { "Journey" } -- vehículo definido para uso del job..
local max_earnings = 308000
local delay = 7
...
local function isJobVehicle( vehicle )
	return vehicle and vehicles[ getElementModel( vehicle ) ] and exports.vehicles:getOwner( vehicle ) == -15 or false
end
		...
		if c then
			if p[ player ].route[ p[ player ].checkpoint ] then
				if p[ player ].route[ p[ player ].checkpoint ].stop and earnings and earnings > 0 then
					...
					exports.factions:giveFactionPresupuesto( 15, 5000 )
				end
...
addEventHandler( "onVehicleEnter", root,
	function( player, seat )
		if seat == 0 and isJobVehicle( source ) and exports.factions:isPlayerInFaction(player, 15) then
