local vehicles = { "Linerunner" } -- vehículo definido para uso del job..
local max_earnings = 1063000
local delay = 6.5
...
local function isJobVehicle( vehicle )
	return vehicle and vehicles[ getElementModel( vehicle ) ] and exports.vehicles:getOwner( vehicle ) == -54 or false
end
		...
		if c then
			if p[ player ].route[ p[ player ].checkpoint ] then
				if p[ player ].route[ p[ player ].checkpoint ].stop and earnings and earnings > 0 then
					...
					exports.factions:giveFactionPresupuesto( 54, 5000 )
				end
...
addEventHandler( "onVehicleEnter", root,
	function( player, seat )
		if seat == 0 and isJobVehicle( source ) and exports.factions:isPlayerInFaction(player, 54) then
				...
				if not p[ source ].route[ p[ source ].checkpoint ].stop or getDistanceBetweenPoints2D( p[ source ].route[ p[ source ].checkpoint ].x, p[ source ].route[ p[ source ].checkpoint ].y, getElementPosition( vehicle ) ) < 10 then
					...
					if health > 350 then
						...
						if getElementData(source, "lastCPBus") == true then
							outputChatBox("Has entregado todos los paquetes, buen trabajo.", source, 0, 195, 0)
