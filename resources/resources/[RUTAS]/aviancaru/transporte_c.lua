...
local function show( )
	...
	if position then
		...
		marker = createMarker( position.x, position.y, position.z, "checkpoint", 18, position.stop and 0 or 255, 255, 0, 60 )
		if marker then
...
addEventHandler( "onClientMarkerLeave", resourceRoot,
	function( element, matching )
		if matching and element == localPlayer and getElementData(getLocalPlayer(), "conductorBus") then
			...
			wait = false
			removeEventHandler( "onClientRender", root, drawWaitingText )
			exports.gui:hint( "Alerta", "Cuidado, mas lento que hay mucho viento", 3 )
		end
	end
)
