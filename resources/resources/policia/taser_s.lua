...
function ejecutarTazerEJC(attacker, weapon, parte)
	if not attacker or not weapon or getElementType(attacker) ~= "player" or not exports.factions:isPlayerInFaction(attacker, 22) then return end
	if (getElementData(attacker, "tazerOn") and weapon == 24) or (getElementData(attacker, "gomaOn") and weapon == 25) then
		if not getElementData(source, "tazed") then
			setElementData (source, "tazed", true)
			setElementFrozen (source, true)
...
addEvent("onPlayerTazed", true)
addEventHandler("onPlayerTazed", getRootElement(), ejecutarTazerEJC)