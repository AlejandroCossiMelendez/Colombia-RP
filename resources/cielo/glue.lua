function glue()
	...
	if not getPedOccupiedVehicle(player) then
		...
		if getElementType(vehicle) == "vehicle" then
			...
			triggerServerEvent("gluePlayerCapitalRP", player, slot, vehicle, x, y, z, rotX, rotY, rotZ)
...
function unglue ()
	local player = getLocalPlayer()
	triggerServerEvent("ungluePlayerCapitalRP", player)
...
function gluev()	
	...
	if myVehicle then
		...
		if getVehicleName(closest) ~= "DFT-30" then return end
		...
		triggerServerEvent("glueVehicleCapitalRP", attachMe, attachedTo, x*-1, y*-1, z*-1, rotX, rotY, rotZ)
...
function ungluev ()
	...
	if myVehicle then
		...
		triggerServerEvent("unglueVehicleCapitalRP", myVehicle)
	end
