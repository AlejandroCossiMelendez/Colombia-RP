function getBalance(player)
	if isElement(player) then
		...
		if currentBalance then
			triggerClientEvent(player, "onClientUpdateBalance", player, currentBalance)
		end
	end
end
...
function ingresarDinero(player, cantidad)
	if exports.players:isLoggedIn(player) then
		...
		if not exports.bank:getDinero(player) then 
			outputChatBox("Se ha producido un error, inténtalo de nuevo más tarde.", player, 255, 0, 0) 
			return 
		end
		...
		if exports.players:takeMoney(player, cantidad) then
			if exports.sql:query_free("UPDATE characters SET banco = " .. (exports.bank:getDinero(player) + cantidad) .. " WHERE characterID = " .. exports.players:getCharacterID(player)) then
				...
				-- Actualizar el balance en el cliente
				triggerClientEvent(player, "onClientUpdateBalance", player, exports.bank:getDinero(player))
				...
				if nivel < 2 then 
					outputChatBox("No puedes usar el banco porque necesitas nivel 2. Usa /objetivos.", player, 255, 0, 0) 
					return 
				end
...
function retirarDinero(player, cantidad)
	if exports.players:isLoggedIn(player) then
		local cantidad = tonumber(math.floor(cantidad))
		if cantidad < 1 then 
		...
		if (exports.bank:getDinero(player) - cantidad) < 0 then 
		...
		if exports.sql:query_free("UPDATE characters SET banco = " .. (exports.bank:getDinero(player) - cantidad) .. " WHERE characterID = " .. exports.players:getCharacterID(player)) then
			...
			outputChatBox("Ha ocurrido un error al procesar tu solicitud.", player, 255, 0, 0)
		end
	end
end
addEvent("onPlayerRemoveBalance", true)
addEventHandler("onPlayerRemoveBalance", root, retirarDinero) 