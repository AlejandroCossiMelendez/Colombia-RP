...
function banearSerial(thePlayer, commandName, serial, horas, ...)
    if not serial or not horas or not (...) then
...
function removeBanSerial(player, cmd, serial)
    ...
    local banquitado = false -- Declarar e inicializar la variable
...
function obtenerTitular(player, cmd, id, tipo)
	if hasObjectPermissionTo(player, "command.acceptreport", false) or exports.factions:isPlayerInFaction(player, 1) or exports.factions:isPlayerInFaction(player, 11) then
		if hasObjectPermissionTo(player, "command.acceptreport", false) and not exports.players:getOption(player, "staffduty") then
			return outputChatBox("Debes estar de servicio (/staffduty) para usar este comando.", player, 255, 0, 0)
		end
		...
		if tipo == 1 then
			...
			if (exports.factions:isPlayerInFaction(player, 1) or exports.factions:isPlayerInFaction(player, 11)) and not exports.players:getOption(player, "staffduty") == true then
				...
				exports.factions:sendMessageToFaction(11, "(( " .. exports.factions:getFactionTag(11) .. " )) " .. getPlayerName(player):gsub("_", " ") .. ": " .. message, 127, 127, 255)
				...
				exports.factions:sendMessageToFaction(11, "(( " .. exports.factions:getFactionTag(11) .. " )) Central: Central para Agente " .. getPlayerName(player):gsub("_", " ") .. ", el titular con matrícula "..tostring(id).." es "..tostring(titular)..".", 127, 127, 255)
			else
		...
		elseif tipo == 2 then
			...
			if (exports.factions:isPlayerInFaction(player, 1) or exports.factions:isPlayerInFaction(player, 11)) and not exports.players:getOption(player, "staffduty") == true then
				...
				exports.factions:sendMessageToFaction(11, "(( " .. exports.factions:getFactionTag(11) .. " )) Central: Central para Agente " .. getPlayerName(player):gsub("_", " ") .. ", el titular del interior "..tostring(id).." es "..tostring(titular)..".", 127, 127, 255)
			else
		...
		elseif tipo == 3 then
			...
			if not sql or not sql.titular then outputChatBox("Este número de teléfono no tiene ningún titular o no existe.", player, 255, 0, 0) return end
			...
			if (exports.factions:isPlayerInFaction(player, 1) or exports.factions:isPlayerInFaction(player, 11)) and not exports.players:getOption(player, "staffduty") == true then
				...
				exports.factions:sendMessageToFaction(11, "(( " .. exports.factions:getFactionTag(11) .. " )) Central: Central para Agente " .. getPlayerName(player):gsub("_", " ") .. ", el titular del número "..tostring(id).." es "..tostring(titular)..".", 127, 127, 255)
			else
...
function setPassword(username, password, adminPlayer)
	local username = tostring(username)
	local password = tostring(password)

	-- Verificar permisos de "Desarrollador"
	if adminPlayer and not isObjectInACLGroup("user."..getAccountName(getPlayerAccount(adminPlayer)), aclGetGroup("Desarrollador")) then
		outputChatBox("No tienes permisos para cambiar contraseñas.", adminPlayer, 255, 0, 0)
		return false
	end
	...
	if username and password then
		...
		if not d or not d.userID then 
			outputChatBox("El usuario '"..username.."' no existe.", adminPlayer, 255, 0, 0)
			return false 
		end

		local newpass = hash("sha1", tostring(salt)..tostring(hash("sha1", tostring(salt)..tostring(hash("sha1", password)))))
		
		-- Imprimir hash generado para depuración
		outputChatBox("Hash generado: "..newpass, adminPlayer, 0, 255, 255)

		local sql, error1 = exports.sql:query_free("UPDATE wcf1_user SET salt = '%s' WHERE userID = " .. d.userID, salt)
		local sql2, error2 = exports.sql:query_free("UPDATE wcf1_user SET password = '%s' WHERE userID = " .. d.userID, newpass)

		if not sql or not sql2 then
			outputChatBox("Error al actualizar la base de datos: "..(error1 or "Desconocido").." / "..(error2 or "Desconocido"), adminPlayer, 255, 0, 0)
			return false
		end
...
function cambiarClaveAdmin(player, cmd, username, nuevaPassword)
	if not username or not nuevaPassword then
		outputChatBox("Syntax: /setclave [usuario] [nueva clave]", player, 255, 255, 255)
		return
	end
	...
	if setPassword(username, nuevaPassword, player) then
		outputChatBox("Has cambiado la clave del usuario "..username.." correctamente.", player, 0, 255, 0)
	else
		outputChatBox("No se pudo cambiar la clave. Contacta con un staff.", player, 255, 0, 0)
	end
end
addCommandHandler("setclave", cambiarClaveAdmin)
...
function borrarMiPJ(player, cmd, con)
	if not con or tostring(con) ~= "ok" then
		...
		exports.logsic:addLogMessage("/pj/"..getPlayerName(player):gsub("_", " "), "PJ Eliminado - cID: "..tostring(exports.players:getCharacterID(player)).." - IP: "..tostring(getPlayerIP(player)).." - Serial: "..tostring(getPlayerSerial(player))..".")
	...
	if hasObjectPermissionTo( player, "command.modchat", false ) then
		if otherPlayer and (...) then
			...
			if other then
				...
				exports.logsic:addLogMessage("listado_ck", "PJ Ckeado: "..name..". Staff: "..exports.players:getUserName(player)..". Razón: "..tostring(razon))
...
-- comando para volver invisible al staff y ocultar el nombre 
addCommandHandler("invisible", function(player)
	if hasObjectPermissionTo(player, "command.modchat", false) then 
		if isElement(player) then 
			if getElementAlpha(player) == 255 then
				setElementAlpha(player, 0)				
				exports.a_infoBox:addBox(player, "Has sido invisible.", "info")
			else
				setElementAlpha(player, 255)				
				exports.a_infoBox:addBox(player, "Has sido visible.", "info")
			end
		end
	end
end)
