...
local customFont = dxCreateFont("fonts/Roboto-Regular.ttf", 12) -- Carga la fuente externa, cambia "path/to/font.ttf" por la ruta real
    ...
    for key, colshape in ipairs(getElementsByType("colshape", resourceRoot)) do
        if getElementDimension(colshape) == dimension then
            ...
            if ownername == localname and not ownageblip[id] and type == 1 then
                ownageblip[id] = createBlip(px, py, pz, 31, 2, 255, 255, 255, 255, 0, 150)
            elseif ownername == localname and not ownageblip[id] and type == 2 then
                ownageblip[id] = createBlip(px, py, pz, 32, 2, 255, 255, 255, 255, 0, 150)
                ...
                if text and (distance < 2 or isLineOfSightClear(cx, cy, cz, px, py, pz + 0.5, true, true, true, true, false, false, true, localPlayer)) then
                    ...
                    if sx and sy then
                        ...
                        if price and type ~= 3 then
                            text = text .. "\nPresiona 'E' para obtener más información sobre su compra."
                        elseif price and type == 3 then
                            text = text .. "\nPresiona 'E' para alquilar por $" .. price .. " cada payday."
                        end
                        local ownerwidth = dxGetTextWidth(tostring(ownertext), 1, customFont)
                        local textwidth = dxGetTextWidth(tostring(text), 1, customFont)
                        local width = math.max(ownerwidth, textwidth)
                        local height = (price and 2 or 1) * dxGetFontHeight(1, customFont)
                        dxDrawRectangle(sx - width, sy - height - 4.8, width * 2, height + 20, tocolor(0, 0, 0, 0))
                        dxDrawText(tostring(text), sx, sy - 27, sx, sy, tocolor(255, 255, 255, 255), 1, customFont, "center", "center", false, false, false, true)
                        dxDrawText(tostring(ownertext), sx, sy + 16, sx, sy, tocolor(255, 197, 0, 255), 1, customFont, "center", "center", false, false, false, true)
                    end
                end
            else
                destroy(colshape)
