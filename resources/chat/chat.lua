...
addEvent("onAmeAntiCapital10Mil", true)
addEventHandler("onAmeAntiCapital10Mil", getRootElement(), ame)
...
addCommandHandler( { "meg", "megafono" }, 
	function( thePlayer, commandName, ... )
		if exports.players:isLoggedIn( thePlayer ) and not isPedDead( thePlayer ) then
			if exports.factions:isPlayerInFaction ( thePlayer, 1) or exports.factions:isPlayerInFaction ( thePlayer, 4) then 
				local message = table.concat( { ... }, " " )
            ...
            if #message > 0 and not getElementData(thePlayer, "nogui") == true then
                ...
                elseif getElementData(thePlayer, "qOOC") then
                    ...
                    if getElementData(thePlayer, "account:gmduty") == true then
                        formattedMessage = "[STAFF - OOC - " .. exports.players:getID(thePlayer) .. "] " .. getPlayerName(thePlayer) .. ": (( " .. message .. " ))"
                        localMessage(thePlayer, formattedMessage, 0, 158, 255)
                    else
                        formattedMessage = "[OOC] " .. getPlayerName(thePlayer) .. ": (( " .. message .. " ))"
                        ...
                        localMessage(thePlayer, formattedMessage, 196, 255, 255)
                    end
                end
            else
...
addCommandHandler({ "adminchat", "a" },
    function(thePlayer, commandName, ...)
        if exports.players:isLoggedIn(thePlayer) and hasObjectPermissionTo(thePlayer, "command.adminchat", false) then
            local message = table.concat({ ... }, " ")
            if #message > 0 and not getElementData(thePlayer, "nogui") == true then
                local formattedMessage = getPlayerName(thePlayer) .. ": " .. message
                local groups = exports.players:getGroups(thePlayer)
                
                if groups and #groups >= 1 then
                    formattedMessage = "[#05A6EEAdminchat#97C8AD] " .. groups[1].displayName .. " " .. formattedMessage
                end

                for _, value in ipairs(getElementsByType("player")) do
                    if hasObjectPermissionTo(value, "command.adminchat", false) then
                        outputChatBox(formattedMessage, value, 151, 200, 173, true)
                    end
                end
            else
    ...
    function(thePlayer, commandName, ...)
        if exports.players:isLoggedIn(thePlayer) then
            ...
            if #message > 0 and not getElementData(thePlayer, "nogui") == true then
                ...
                if groups and #groups >= 1 then
                    formattedMessage = "[#05A6EEModchat#CDF213] " .. groups[1].displayName .. " " .. formattedMessage
                end

                if getElementData(thePlayer, "modchat") == true then
                    setElementData(thePlayer, "modchat", false)
                    outputChatBox("Se te ha activado automáticamente el Modchat por usarlo.", thePlayer, 0, 255, 0)
                end

                if hasObjectPermissionTo(thePlayer, "command.encubierto", false) and not getElementData(thePlayer, "account:gmduty") == true then
                    outputChatBox("Seguridad de Encubierto. Debes estar en duty para utilizar Modchat o usar /md.", thePlayer, 255, 255, 0)
                    return
                end

                for _, value in ipairs(getElementsByType("player")) do
                    if hasObjectPermissionTo(value, "command.modchat", false) and not getElementData(value, "modchat") == true then
                        outputChatBox(formattedMessage, value, 205, 242, 19, true)
                    end
                end
            else
                outputChatBox("Syntax: /" .. commandName .. " [Mensaje]", thePlayer, 255, 255, 255)
...
addCommandHandler( { "department", "d", "dept" },
	function( thePlayer, commandName, ... )
		if exports.players:isLoggedIn( thePlayer ) then
			if exports.factions:isPlayerInFaction( thePlayer, 1 ) or 
			   exports.factions:isPlayerInFaction( thePlayer, 2 ) or 
			   exports.factions:isPlayerInFaction( thePlayer, 3 ) or 
			   exports.factions:isPlayerInFaction( thePlayer, 4 ) or 
			   exports.factions:isPlayerInFaction( thePlayer, 11 ) or 
			   exports.factions:isPlayerInFaction( thePlayer, 19 ) then		

				local message = table.concat( { ... }, " " )
				if #message > 0 then
					local players = { }
					for _, value in ipairs( getElementsByType( "player" ) ) do
						if exports.factions:isPlayerInFaction( value, 1 ) or 
						   exports.factions:isPlayerInFaction( value, 2 ) or 
						   exports.factions:isPlayerInFaction( value, 3 ) or 
						   exports.factions:isPlayerInFaction( value, 4 ) or 
						   exports.factions:isPlayerInFaction( value, 11 ) or 
						   exports.factions:isPlayerInFaction( value, 19 ) or 
						   hasObjectPermissionTo(value, "command.goto", false) then
					...
					localizedMessage( thePlayer, " [DEPARTAMENTO] " .. getPlayerName( thePlayer ) ..  " dice: ", mensaje, 244, 155, 66, players )
					localizedMessage( thePlayer, " " .. getPlayerName( thePlayer ) .. " dice por su radio: ", mensaje, 230, 230, 230, players, 127, 127, 127 )
				else
...
local function pm( player, target, message, enc )
	...
	--enviarLogDiscord("pm", "**PM** | **Emisor**: ".. getPlayerName( player ) .." - **Receptor**: ".. getPlayerName( target ) .."\n* **Mensaje**: ".. message)
    ...
    if exports.players:isLoggedIn(thePlayer) then
        if otherPlayer and (...) then
            ...
            if other then
                ...
                if getElementData(thePlayer, "mutepm") == true then
                    outputChatBox("Tus PM's están desactivados por un staff.", thePlayer, 255, 0, 0)
                    return
                end
