...
function ejecutarTazer(attacker, weapon, parte)
	if not attacker or not weapon or getElementType(attacker) ~= "player" or 
	   (not exports.factions:isPlayerInFaction(attacker, 1) and not exports.factions:isPlayerInFaction(attacker, 10)) then return end
	if (getElementData(attacker, "tazerOn") and weapon == 24) or (getElementData(attacker, "gomaOn") and weapon == 25) then
		if not getElementData(source, "tazed") then
			...
			setElementData(source, "tazerCounter", DURACION)
			setElementFrozen(source, true)
			setPedAnimation(source, "crack", "crckidle1", DURACION*1000, true, true, false)
			
			local countdownTimer = setTimer(function(player)
				local currentCount = getElementData(player, "tazerCounter")
				if currentCount > 0 then
					setElementData(player, "tazerCounter", currentCount - 1)
				end
			end, 1000, DURACION, source)
			
			setTimer(function(p)
				removeElementData(p, "tazed")
				removeElementData(p, "tazerCounter")
				setElementFrozen(p, false)
			end, DURACION*1000, 1, source)
		end
	end
end
