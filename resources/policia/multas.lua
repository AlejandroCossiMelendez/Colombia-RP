function multar ( thePlayer, commandName, otherPlayer, precio, ... )
	if exports.factions:isPlayerInFaction( thePlayer, 1 ) or exports.factions:isPlayerInFaction( thePlayer, 11 )  or exports.factions:isPlayerInFaction( thePlayer, 22 ) then
...
function multar2 ( thePlayer, commandName, otherPlayer, precio, ... )
	if exports.factions:isPlayerInFaction( thePlayer, 1 ) or exports.factions:isPlayerInFaction( thePlayer, 11 ) or getElementData(thePlayer, "enc1") == true then
		if thePlayer and otherPlayer and precio and ... then
			local razon = table.concat({...}, " ")
			local otro, nombre = exports.players:getFromName(thePlayer, otherPlayer)
			if not otro then return end
...
function multarveh ( thePlayer, commandName, idveh, precio, ... )
	if exports.factions:isPlayerInFaction( thePlayer, 1 ) or exports.factions:isPlayerInFaction( thePlayer, 11 ) or getElementData(thePlayer, "enc1") == true then
		if thePlayer and idveh and precio and (...) then
			...
			if tostring(characterID) == "nil" then 
				outputChatBox("ID del vehículo incorrecto, o el vehículo ha sido desguazado.", thePlayer, 255, 0, 0) 
				return 
			end
