	...
	if userID and groupID and groupID > 3 and (groupID < 12 or groupID > 14) then
		...
		if sql then
			...
			if result.number == 0 then
				...
				exports.logsic:addLogMessage("logFORO2", "AVERIA uID "..tostring(userID).." faction "..tostring(sql.factionID))
				return false
			else
				return true
			end
...
function actualizarPermisosEnForo(userID)
	...
	if c3 then
		...
		for k, v in ipairs(c3) do
			if tieneFaccionAsociada(tonumber(v.groupID)) then
				if tienePJEnEseGrupo(tonumber(userID), tonumber(v.groupID)) then
					...
					exports.logsic:addLogMessage("error-foro-permisos", "uID "..tostring(userID).." no se pudo actualizar permisos de foro E1.")
					actualizarPermisosEnForo(userID)
					return
				end
			else
		...
		if tonumber(sql.id_group) then
			...
			exports.logsic:addLogMessage("error-foro-permisos", "uID "..tostring(userID).." no se pudo actualizar permisos de foro E2.")
			actualizarPermisosEnForo(userID)
			return
		end
	end
