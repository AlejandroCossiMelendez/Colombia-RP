...
local mostrarMensaje = false

addEventHandler("onClientRender", root, function()
    local arma = getPedWeapon(jugador) -- Obtiene el arma actual
    local balasEnCargador = getPedAmmoInClip(jugador) -- Balas en el cargador
    local balasTotales = getPedTotalAmmo(jugador) -- Balas en total (cargador + reserva)

    if balasEnCargador == 1 and balasTotales == 1 then
        toggleControl("fire", false) -- Bloquea el disparo
        mostrarMensaje = true -- Activa el mensaje de recarga
    else
        toggleControl("fire", true) -- Permite disparar si tiene más de 1 bala
        mostrarMensaje = false -- Oculta el mensaje si recarga
    end

    -- Dibujar mensaje en pantalla si debe recargar
...
function desactivarDisparoSinBalas ( prevSlot, newSlot )
	...
	if isArmaBalas[getPedWeapon(getLocalPlayer(),newSlot)] and balasFromArma <= 2 then
...
-- Normalizador de valor por tipo de item (client)
