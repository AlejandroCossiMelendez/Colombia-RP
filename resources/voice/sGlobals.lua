if isVoiceEnabled() then
	TEAM_REFRESH = 2000 -- Interval in which team channels are refreshed, in MS.
...
addEventHandler("onCharacterLogin", getRootElement(), function()
	triggerClientEvent(source, "voice:ConfigurarVozAlEntrar", source, source)
end)

addEventHandler("onResourceStart", resourceRoot, function()
	setTimer(function()
		for _, player in pairs(getElementsByType("player")) do
			triggerClientEvent(player, "voice:ConfigurarVozAlEntrar", player, player)
		end
	end, 2000, 1)
end)