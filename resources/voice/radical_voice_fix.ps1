...
$backup = $p + '.bak.radical.' + (Get-Date).ToString('yyyyMMddHHmmss')
Copy-Item $p $backup
...
-- =====================================================
-- SOLUCIÓN RADICAL: GUI Labels en lugar de dxDraw
-- =====================================================
...
-- Crear GUI Labels (mucho más eficiente que dxDraw)
...
function moverBones()
    ...
    -- Solo procesar 1 jugador máximo por segundo
    for player in pairs(voicePlayers) do
...
Set-Content -Encoding UTF8 $p $radicalOptimization
Write-Host "RADICAL voice optimization applied - GUI Labels instead of dxDraw. Should drop below 0.5%. Backup: $backup"
