...
bindKey("F2", "down", -- Enlaza la tecla F5 para detectar cuando se presiona
function()
    -- Verificar si el jugador está marcado como muerto
    if getElementData(localPlayer, "muerto") == true then
        -- Si está muerto, no se permite abrir el panel de animaciones
        outputChatBox("¡No puedes abrir el panel de animaciones porque estás muerto!", 255, 0, 0)
        return
    end

    -- Verificar si el evento "onClientRender" está agregado para la función "dx"
    if isEventHandlerAdded("onClientRender", root, dx) then
        -- Si está agregado, remueve el evento para detener la renderización de la función "dx"
        removeEventHandler("onClientRender", root, dx)
        showCursor(false) -- Oculta el cursor del ratón
    else
        -- Si no está agregado, agrega el evento para iniciar la renderización de la función "dx"
        addEventHandler("onClientRender", root, dx)
        showCursor(true) -- Muestra el cursor del ratón
        window  = "index" -- Inicializa algunas variables relacionadas con el panel de animaciones
        proxPage = 0
        selectButton = 0
        selectAnim = 0
        if not selectFav then selectFav = {} end -- Verifica si la variable "selectFav" existe, si no, la inicializa
        buttonSelect = nil
        AnimSelecionada = nil
        proxPageSelect = 0
        tick = getTickCount() -- Obtiene el tiempo actual en milisegundos
    end
end)
...
function()
    ...
    if getElementData(localPlayer, "muerto") == true then
        -- Si está muerto, no se ejecuta la animación
        outputChatBox("¡No puedes realizar la animación porque estás muerto!", 255, 0, 0)
        return
    end

    -- Si no está muerto, se ejecuta la animación
