function isElementInRange(ele, x, y, z, range)
 ...
 -- Eventos para controlar el vehículo desde la interfaz
 ...
 addEventHandler("vehicles:toggleEngine", root, function()
	 if client then
		 local vehicle = getPedOccupiedVehicle(client)
		 if vehicle and getVehicleOccupant(vehicle) == client then
			 if getElementData(vehicle, "cepo") then
				 exports.chat:ame(client, "Vehículo", "Podrías ver que tiene un cepo policial en la rueda.")
				 exports.chat:me(client, "intenta arrancar pero no puede.")
				 return
			 end
			 if getElementHealth(vehicle) <= 359 then
				 outputChatBox("El motor ha reventado.", client, 255, 0, 0)
				 return
			 end
             if getElementData(vehicle, "fuel") <= 0 then
				 setVehicleEngineState(vehicle, false)
				 triggerClientEvent(client, "gui:hint", client, "Sin gasolina", "Tu " .. getVehicleName(vehicle) .. " No arranca sin gasolina!\nPara que no te vuelva a pasar esto recarga cada un tiempo.", 3)
				 return
			 end
             if exports["carrobomba"] and exports["carrobomba"].attemptEngineStart and exports["carrobomba"]:attemptEngineStart(client, vehicle) == false then
                 return
             end
             setVehicleEngineState(vehicle, not getVehicleEngineState(vehicle))
			 exports.chat:me(client, (getVehicleEngineState(vehicle) and "enciende" or "apaga") .. " el motor.")
		 end
	 end
 end)
 
 -- Función para encender/apagar las luces
 addEventHandler("vehicles:toggleLights", root, function()
	 if client then
		 local vehicle = getPedOccupiedVehicle(client)
		 if vehicle and getVehicleOccupant(vehicle) == client then
			 setVehicleOverrideLights(vehicle, getVehicleOverrideLights(vehicle) == 2 and 1 or 2)
			 exports.chat:me(client, (getVehicleOverrideLights(vehicle) == 2 and "enciende" or "apaga") .. " las luces.")
		 end
	 end
 end)
 
 -- Función para el freno de mano
 addEventHandler("vehicles:toggleHandbrake", root, function()
	 if client then
		 local vehicle = getPedOccupiedVehicle(client)
		 if vehicle and getVehicleOccupant(vehicle) == client then
			 if isVehicleOnGround(vehicle) then
				 ...
				 if actualspeed <= 0.05 then
					 ...
					 if getVehicleType(vehicle) == "Bike" or getVehicleType(vehicle) == "BMX" then
						 exports.chat:me(client, (frozen and "quita" or "pone") .. " la pata de cabra.")
					 else
						 exports.chat:me(client, (frozen and "quita" or "pone") .. " el freno de mano.")
					 end
				 end
			 end
		 end
 ...
 -- Evento para manejar los mensajes de las puertas
 addEvent("vehicles:doorAction", true)
 addEventHandler("vehicles:doorAction", root, function(doorName, isOpening)
	 local action = isOpening and "abre" or "cierra"
	 exports.chat:me(source, action .. " la puerta " .. doorName)
 end)