...
function resetDrug2Effect()
	...
	setGameSpeed(1)
	setGravity (0.008)
	setElementHealth(getLocalPlayer(), getElementHealth(getLocalPlayer())-20)
	triggerServerEvent("onAme", getLocalPlayer(), getLocalPlayer(), getPlayerName(getLocalPlayer()):gsub("_", " "), "Te sientes agotado, con dificultades para orinar y posible impotencia sexual")
	setElementData(getLocalPlayer(), "cansancio", tonumber(getElementData(getLocalPlayer(), "cansancio")-70))
end
...
function doDrug3Effect()
		...
		drug3timer = setTimer(resetDrug3Effect, 300000, 1)
		...
		setGameSpeed(0.8)
		triggerServerEvent("onAme", getLocalPlayer(), getLocalPlayer(), getPlayerName(getLocalPlayer()):gsub("_", " "), "Puedes sentirte adormilado, con la boca seca y pastosa junto a un ligero apetito")
		outputChatBox ( "Esta droga dura 5 minutos.", 255, 0, 0 )
		setElementData(getLocalPlayer(), "cansancio", tonumber(getElementData(getLocalPlayer(), "cansancio")-70))
		setElementData(getLocalPlayer(), "sed", tonumber(getElementData(getLocalPlayer(), "sed")-30))
		setElementData(getLocalPlayer(), "hambre", tonumber(getElementData(getLocalPlayer(), "hambre")-15))
end
