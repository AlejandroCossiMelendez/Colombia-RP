                    ...
                    if responsavelconvite and isElement(responsavelconvite) then 
                        ...
                        for i,v in pairs(convitesPlayer[responsavelconvite]) do 
                            if not v and i and isElement(i) then 
                                notifyS(responsavelconvite, "El jugador "..removeHex (getPlayerName(i)).." no aceptó la invitación", "error")
                                variavelrecusar = true
                            elseif not v then 
                                variavelrecusar = true
                                notifyS(responsavelconvite, "¡Un jugador invitado se desconectó!", "error")
                            end 
                        end 
...
function setAnim(player, nome, anim, tipo, int)
    if tipo == 'ESTILO DE CAMINAR' then
        if not isPedInVehicle(player) then
            setPedWalkingStyle(player, tonumber(anim))
            notifyS(player, 'Cambiaste tu estilo de caminar a '..nome..'.', 'info')
        end
    elseif tipo ~= 'ESTILO DE CAMINAR' then 
        if int == 'IFP' then 
            triggerClientEvent(root, 'Schootz.onSetAnimation', root, player, anim)
            notifyS(player, 'Activaste la animación '..nome..'.', 'info')
    elseif int == 'Custom' then
        ...
        if table then
            ...
            if table.Object then
                ...
                --notifyS(player, 'Has tomado el objeto '..nome..'.', 'info')
            end
        end
